...
class RebotlingController {
    ...
    private function getCycleTrend() {
        try {
            ...
            if ($granularity === 'shift') {
                // Per-skift granularitet — ibc_ok/ibc_ej_ok/bur_ej_ok reset per skiftraknare,
                // so MAX() per skiftraknare gives the correct per-shift total. No LAG needed.
                $stmt = $this->pdo->prepare("
                    SELECT DATE(datum)                    AS dag,
                           skiftraknare,
            ...
            // ibc_ok/ibc_ej_ok/bur_ej_ok reset per skiftraknare — MAX() per skiftraknare
            // gives the correct per-shift total. Aggregate to daily by summing across shifts.
            $stmt = $this->pdo->prepare("
                SELECT
                    dag,
                    SUM(cykler)                                                       AS cycles,
                    ROUND(AVG(avg_runtime), 1)                                        AS avg_runtime,
                    ROUND(SUM(ibc_ok) * 60.0 / NULLIF(SUM(shift_runtime), 0), 1)    AS avg_ibc_per_hour,
                    SUM(ibc_ok)    AS total_ibc_ok,
                    SUM(ibc_ej_ok) AS total_ibc_ej_ok,
                    SUM(bur_ej_ok) AS total_bur_ej_ok
                FROM (
    ...
    private function getLiveRanking() {
        try {
            ...
            // ibc_ok resets per skiftraknare — MAX() per skiftraknare gives the correct
            // per-shift total. No LAG needed. UNION ALL for op1/op2/op3 attribution.
            $sql = "
                WITH lag_shifts AS (
                    SELECT skiftraknare,
                ...
                SELECT
                    ...
                    COALESCE(o.name, CONCAT('Operatör ', t.op_num)) AS name,
                    SUM(t.ibc_ok)   AS ibc_ok,
                    SUM(t.totalt)   AS totalt,
                    SUM(t.drifttid) AS drifttid,
                    COUNT(t.skiftraknare) AS shifts_count
                FROM (
                    SELECT skiftraknare, op1 AS op_num, ibc_ok, totalt, drifttid FROM lag_shifts WHERE op1 IS NOT NULL AND op1 > 0
                    UNION ALL
                    SELECT skiftraknare, op2, ibc_ok, totalt, drifttid FROM lag_shifts WHERE op2 IS NOT NULL AND op2 > 0
                    UNION ALL
                    SELECT skiftraknare, op3, ibc_ok, totalt, drifttid FROM lag_shifts WHERE op3 IS NOT NULL AND op3 > 0
                ) t
                LEFT JOIN operators o ON o.number = t.op_num
                GROUP BY t.op_num, o.name
                ORDER BY (SUM(t.ibc_ok) / GREATEST(SUM(t.drifttid)/60, 0.01)) DESC
                LIMIT 10
            ";

            // :dateFrom appears once in the CTE inner query → single positional param
            ...
            // ibc_ok resets each shift (skiftraknare). Correct total = MAX(ibc_ok) per
            // (DATE, skiftraknare) — then SUM those per-shift maxima across all shifts in
            // the period. Plain MAX(ibc_ok) over the whole period gives a single-shift
            // ceiling and under-counts multi-shift days. DATE() groups are needed for the
            // 7-day historik fallback so we don't sum the same shift twice.
            ...
            try {
                $stmtRecord = $this->pdo->prepare("
                    ...
                    FROM (
                        ...
                        FROM (
                            SELECT skiftraknare,
                                   DATE(MAX(datum))            AS datum_rekord,
        ...
        try {
            ...
            $sql = "
                ...
                SELECT
                    ...
                    COUNT(DISTINCT t.skiftraknare) AS total_skift
                FROM (
                    SELECT skiftraknare, op1 AS op_num, ibc_ok, totalt, drifttid FROM lag_shifts WHERE op1 IS NOT NULL AND op1 > 0 AND ibc_ok > 0
                    UNION ALL
                    SELECT skiftraknare, op2, ibc_ok, totalt, drifttid FROM lag_shifts WHERE op2 IS NOT NULL AND op2 > 0 AND ibc_ok > 0
                    UNION ALL
                    SELECT skiftraknare, op3, ibc_ok, totalt, drifttid FROM lag_shifts WHERE op3 IS NOT NULL AND op3 > 0 AND ibc_ok > 0
                ) t
                LEFT JOIN operators o ON o.number = t.op_num
                GROUP BY t.op_num, o.name
            ...
            // ibc_ok resets per skiftraknare — MAX() per skiftraknare gives the correct
            // per-shift total. No LAG needed.
            $stmtAllShifts = $pdo->query("
                SELECT skiftraknare,
                       DATE(MAX(datum))            AS dag,
                       MAX(COALESCE(op1, 0))       AS op1,
                       MAX(COALESCE(op2, 0))       AS op2,
                       MAX(COALESCE(op3, 0))       AS op3,
                       MAX(COALESCE(ibc_ok,    0)) AS ibc_delta
                FROM rebotling_skiftrapport
                WHERE ibc_ok IS NOT NULL
                GROUP BY skiftraknare
            ");
            $allShiftRows = $stmtAllShifts->fetchAll(\PDO::FETCH_ASSOC);
            ...
            foreach ($allShiftRows as $s) {
                $dag   = $s['dag'];
                $delta = max(0, (int)$s['ibc_delta']);
                if ($delta <= 0) continue;

                $dt = new \DateTime($dag);
                $yw = $dt->format('Y') . '-' . str_pad($dt->format('W'), 2, '0', STR_PAD_LEFT);
                $ym = $dt->format('Y-m');

                $teamDays[$dag]  = ($teamDays[$dag]  ?? 0) + $delta;
                $teamWeeks[$yw]  = ($teamWeeks[$yw]  ?? 0) + $delta;
                $teamMonths[$ym] = ($teamMonths[$ym] ?? 0) + $delta;

                foreach (['op1', 'op2', 'op3'] as $pos) {
                    $opNum = (int)$s[$pos];
                    if ($opNum <= 0) continue;
                    $opDays[$opNum][$dag] = ($opDays[$opNum][$dag] ?? 0) + $delta;
                    $opWeeks[$opNum][$yw] = ($opWeeks[$opNum][$yw] ?? 0) + $delta;
                    $opMonths[$opNum][$ym] = ($opMonths[$opNum][$ym] ?? 0) + $delta;
                }
            }

            $bestDayByOp = [];
            foreach ($opDays as $opNum => $days) {
                arsort($days);
                $bestDate = array_key_first($days);
                $bestDayByOp[$opNum] = ['ibc' => $days[$bestDate], 'date' => $bestDate];
            }

            $bestWeekByOp = [];
            foreach ($opWeeks as $opNum => $weeks) {
                $bestWeekByOp[$opNum] = max($weeks);
            }

            $bestMonthByOp = [];
            foreach ($opMonths as $opNum => $months) {
                $bestMonthByOp[$opNum] = max($months);
            }

            $teamBestDay   = $teamDays   ? max($teamDays)   : 0;
            $teamBestWeek  = $teamWeeks  ? max($teamWeeks)  : 0;
            $teamBestMonth = $teamMonths ? max($teamMonths) : 0;
    ...
    private function getHallOfFameDays() {
        try {
            ...
            // ibc_ok/totalt reset per skiftraknare — MAX() per skiftraknare gives the
            // correct per-shift total. Sum across shifts to get daily totals.
            $sql = "
                SELECT dag, SUM(ibc_ok) AS ibc_total,
                       ROUND(100.0 * SUM(ibc_ok) / NULLIF(SUM(totalt), 0), 1) AS avg_quality
                FROM (
                    SELECT skiftraknare,
                           DATE(MAX(datum))            AS dag,
                           MAX(COALESCE(ibc_ok,    0)) AS ibc_ok,
    ...
    private function getMonthlyLeaders() {
        try {
            ...
            // ibc_ok/ibc_ej_ok reset per skiftraknare — MAX() per skiftraknare gives the
            // correct per-shift total. No LAG needed.
            $sql = "
                SELECT
                    DATE_FORMAT(DATE(MAX(datum)), '%Y-%m')                       AS manad,
                    SUM(ibc_ok)                                                  AS total_ibc,
                    SUM(ibc_ok + ibc_ej_ok)                                      AS total_all,
                    SUM(drifttid_min)                                            AS runtime_min,
                    SUM(rasttime_min)                                            AS rast_min,
                    MAX(ibc_ok / GREATEST(drifttid_min / 60.0, 0.01))           AS top_ibc_h
                FROM (
                    SELECT skiftraknare,
                           MAX(datum)                      AS datum,
                           MAX(COALESCE(ibc_ok,    0)) AS ibc_ok,
                           ...
                           MAX(COALESCE(rasttime,  0)) AS rasttime_min
                    FROM rebotling_skiftrapport
    ...
    private function getOperatorWeeklyTrend(): void {
        ...
        try {
            ...
            $sql = "
                SELECT
                    YEAR(DATE(MAX(datum)))                                    AS yr,
                    WEEK(DATE(MAX(datum)), 3)                                 AS wk,
                    MIN(CONCAT('V.', LPAD(WEEK(DATE(MAX(datum)), 3), 2, '0'))) AS vecka_label,
                    SUM(ibc_ok)                                              AS total_ibc,
                ...
                FROM (
                    SELECT skiftraknare,
                           ...
                           MAX(COALESCE(drifttid, 0))  AS drifttid_min
                    FROM rebotling_skiftrapport
                    WHERE datum >= ? AND ibc_ok > 0
                    GROUP BY skiftraknare
                    HAVING MAX(COALESCE(drifttid, 0)) >= 30
                ) base
                WHERE drifttid_min >= 30 AND (op1 = ? OR op2 = ? OR op3 = ?)
                GROUP BY yr, wk
    ...
    private function getOeeComponents() {
        ...
        try {
            // ibc_ok/ibc_ej_ok reset per skiftraknare — MAX() per skiftraknare gives the
            // correct per-shift total. Aggregate to daily by summing across shifts.
            $sql = "
                SELECT
                    dag,
                    SUM(ibc_ok)      AS daily_ibc_ok,
                    SUM(ibc_ej_ok)   AS daily_ibc_ej_ok,
            ...
            FROM (
                ...
                FROM (
                    SELECT skiftraknare, DATE(MAX(datum)) AS dag,
                           ...
                           MAX(COALESCE(bur_ej_ok,  0)) AS shift_bur,
    ...
    private function queryOperatorWeeklyTrend(int $opNr, string $from, string $to): array {
        ...
        // per-shift total. No LAG needed.
        $sql = "
            SELECT YEARWEEK(DATE(MAX(datum)), 1) AS yw,
    ...
    private function getShiftDna(): void {
        try {
            ...
            // Team average IBC/h over last 90 days — LAG() per-shift delta (running counter)
    ...
    private function getBonusKalkylator(): void
    {
        try {
            ...
            $stmt = $pdo->prepare("
                SELECT skiftraknare, datum AS datum, op1, op2, op3, drifttid,
                       ibc_ok
                FROM (
                    SELECT skiftraknare,
                           DATE(MAX(datum))        AS dag,
    ...
    private function getOperatorInlarning(): void {
        try {
            ...
            // Fetch all shifts with per-shift IBC delta via LAG() — ibc_ok is a PLC daily
            // running counter; LAG() gives the per-shift contribution, not the cumulative total.
            // Single query drives both team averages (PHP-computed) and per-operator learning curves.
            $stmt = $this->pdo->prepare("
                SELECT datum, op1, op2, op3, drifttid,
                       ibc_ok
                FROM (
                    SELECT skiftraknare,
                           DATE(MAX(datum))   AS datum,
            ...
            // Compute team avg IBC/h per position from the LAG-corrected shifts
            $teamPosIbc = [1 => 0.0, 2 => 0.0, 3 => 0.0];
            $teamPosMin = [1 => 0.0, 2 => 0.0, 3 => 0.0];
            foreach ($allShifts as $r) {
                $ibc = max(0, (float)$r['ibc_ok']); $min = max(0, (float)$r['drifttid']);
                if ($min < 30) continue;
                if ((int)$r['op1'] > 0) { $teamPosIbc[1] += $ibc; $teamPosMin[1] += $min; }
                if ((int)$r['op2'] > 0) { $teamPosIbc[2] += $ibc; $teamPosMin[2] += $min; }
                if ((int)$r['op3'] > 0) { $teamPosIbc[3] += $ibc; $teamPosMin[3] += $min; }
            }
            $teamAvg = [
                1 => $teamPosMin[1] > 0 ? round($teamPosIbc[1] / ($teamPosMin[1] / 60.0), 1) : 0.0,
                2 => $teamPosMin[2] > 0 ? round($teamPosIbc[2] / ($teamPosMin[2] / 60.0), 1) : 0.0,
                3 => $teamPosMin[3] > 0 ? round($teamPosIbc[3] / ($teamPosMin[3] / 60.0), 1) : 0.0,
            ];
    ...
    private function getOperatorVarning(): void {
        try {
            ...
            // ibc_ok is a PLC daily running counter — LAG() computes per-shift delta
            $stmt = $this->pdo->prepare("
                SELECT op1, op2, op3, drifttid, datum,
                       ibc_ok
                FROM (
                    SELECT skiftraknare,
                           DATE(MAX(datum)) AS datum,
    ...
    private function getOperatorProdukt(): void {
        try {
            ...
            // ibc_ok is a PLC daily running counter — LAG() computes per-shift delta
            $stmt = $this->pdo->prepare("
                SELECT op1, op2, op3, drifttid, product_id,
                       ibc_ok
                FROM (
                    SELECT skiftraknare,
                           DATE(MAX(datum))  AS datum,
    ...
    private function getSkiftKalender(): void {
        ...
        try {
            ...
            $stmt = $pdo->prepare("
                SELECT skiftraknare, datum, op1, op2, op3, drifttid, driftstopptime, product_id,
                       ibc_ok,
                       ibc_ej_ok
                FROM (
                    SELECT skiftraknare,
    ...
    private function getOperatorVeckodag(): void {
        ...
        try {
            ...
            // Single LAG()-corrected query for all shifts — drives both team avg and per-operator DOW.
            // ibc_ok is a PLC daily running counter; LAG() computes the per-shift delta.
            // Position filter applied AFTER LAG() so no shifts are dropped from the window.
            $allShiftsStmt = $pdo->prepare("
                SELECT datum, op1, op2, op3, drifttid,
            ...
            ");
            $allShiftsStmt->execute([':from' => $from, ':to' => $to]);
            $allShifts = $allShiftsStmt->fetchAll(\PDO::FETCH_ASSOC);

            // Build team avg per DOW and per-operator per-DOW from PHP
            $teamDowIbc = []; $teamDowMin = []; $teamDowCnt = [];
            $opDowIbc   = []; $opDowMin   = []; $opDowCnt   = [];
            foreach ($allShifts as $s) {
                $ibc = max(0, (float)$s['ibc_ok']); $min = max(0, (float)$s['drifttid']);
                if ($min <= 0) continue;
                $dow = (int)$s['dow'];
                // Team
                $teamDowIbc[$dow] = ($teamDowIbc[$dow] ?? 0) + $ibc;
                $teamDowMin[$dow] = ($teamDowMin[$dow] ?? 0) + $min;
                $teamDowCnt[$dow] = ($teamDowCnt[$dow] ?? 0) + 1;
                // Per operator (each position independently)
                foreach ([1,2,3] as $p) {
                    $num = (int)$s["op$p"];
                    if ($num <= 0 || !isset($opMap[$num])) continue;
                    $opDowIbc[$num][$dow] = ($opDowIbc[$num][$dow] ?? 0) + $ibc;
                    $opDowMin[$num][$dow] = ($opDowMin[$num][$dow] ?? 0) + $min;
                    $opDowCnt[$num][$dow] = ($opDowCnt[$num][$dow] ?? 0) + 1;
                }
            }
            ...
            $teamWeightedSum = 0.0;
            $teamShiftsTotal = 0;
            foreach ($teamDowIbc as $dow => $ibcSum) {
                $minSum = $teamDowMin[$dow] ?? 0;
                $cnt    = $teamDowCnt[$dow] ?? 0;
                $avg    = $minSum > 0 ? round($ibcSum / ($minSum / 60.0), 2) : 0.0;
            ...
            $byOp = [];
            foreach ($opDowIbc as $num => $dowMap) {
                foreach ($dowMap as $dow => $ibcSum) {
                    $minSum = $opDowMin[$num][$dow] ?? 0;
                    $cnt    = $opDowCnt[$num][$dow] ?? 0;
                    if ($cnt < 2) continue;
                    $avgIbcH = $minSum > 0 ? round($ibcSum / ($minSum / 60.0), 2) : 0.0;
                    $vsTeam  = (isset($teamByDow[$dow]) && $teamByDow[$dow]['team_avg'] > 0)
                        ? round($avgIbcH / $teamByDow[$dow]['team_avg'] * 100, 1)
                        : null;
                    if (!isset($byOp[$num])) $byOp[$num] = [];
                    $byOp[$num][$dow] = ['avg_ibc_h' => $avgIbcH, 'shifts' => $cnt, 'vs_team' => $vsTeam];
                }
            }
    ...
    private function getSkiftPrognos(): void
    {
        try {
            ...
            foreach ($positions as $idx => $pos) {
                ...
                // ibc_ok is a PLC daily running counter — LAG() gives per-shift delta.
                // All shifts in day included in LAG window; col filter applied in outer WHERE.
                // ibc_ok resets per skiftraknare — MAX() per skiftraknare gives the correct
                // per-shift total. No LAG needed.
                $teamSql = "
                    ...
                    FROM (
                        SELECT op_at_pos,
                               CASE WHEN drifttid > 0
                                    THEN ibc_ok / (drifttid / 60.0)
                                    ELSE 0 END AS shift_ibc_h
                        FROM (
                            SELECT skiftraknare,
                                   DATE(MAX(datum))     AS dag,
                                   MAX({$col})          AS op_at_pos,
                                   MAX(ibc_ok)          AS ibc_ok,
                                   MAX(drifttid)        AS drifttid
                            FROM rebotling_skiftrapport
                    ...
                    WHERE op_at_pos IS NOT NULL AND shift_ibc_h > 0
                ";
                ...
                if ($opNum !== null && isset($opMap[$opNum])) {
                    ...
                    // LAG() window over all shifts of the day; filter to this operator in outer WHERE.
                    $opSuffix = "op{$suffix}";
                    $opSql = "
                        SELECT shift_ibc_h
                        FROM (
                        ...
                        WHERE op_at_pos = :{$opSuffix}_num AND shift_ibc_h > 0
                    ";
    ...
    private function getIbcForlust(): void
    {
        try {
            ...
            // All unique shifts in period — ibc_ok is a PLC daily running counter,
    ...
    private function getSkiftTopplista(): void {
        try {
            ...
            // Deduplicated shifts with per-shift IBC delta via LAG()
            // ibc_ok/ibc_ej_ok are PLC daily running counters — later shifts would get
            // cumulative totals without LAG(); ORDER BY in outer_q after delta computation
            $sql = "
                SELECT skiftraknare, datum, op1, op2, op3, drifttid, driftstopptime, ibc_ok, ibc_ej_ok
                FROM (
                ...
                WHERE ibc_ok >= 1
                ORDER BY ibc_ok / (drifttid / 60.0) DESC
            ";
            $stmt = $this->pdo->prepare($sql);
    ...
    private function getProduktionHeatmap(): void
    {
        try {
            ...
            // Aggregate per day: ibc_ok is a PLC daily running counter so MAX per day
            // gives the true daily total; SUM(shift drifttid) gives total runtime
            $stmt = $this->pdo->prepare("
                SELECT
                    datum,
                    MAX(ibc_ok)   AS day_ibc,
    ...
    private function getRekordsStatistik(): void {
        try {
            ...
            // ibc_ok is a PLC daily running counter — LAG() gives per-shift delta so later
            // shifts in a multi-shift day don't inherit the cumulative total.
            $bestSql = "
                SELECT
                    t.skiftraknare,
                    t.datum,
                ...
                FROM (
                    SELECT skiftraknare, datum, drifttid, op1, op2, op3,
                           ibc_ok,
                           ibc_ok / NULLIF(drifttid / 60.0, 0) AS ibc_per_h
                    FROM (
                        SELECT skiftraknare, datum, drifttid, op1, op2, op3,
                               ibc_ok
                        FROM (
                    ...
                    WHERE ibc_ok > 0 AND drifttid >= 30
                ) ranked
                LEFT JOIN operators o1 ON o1.number = ranked.op1
                LEFT JOIN operators o2 ON o2.number = ranked.op2
                LEFT JOIN operators o3 ON o3.number = ranked.op3
            ...
            // ibc_ok is a daily PLC running counter — LAG() gives correct per-shift delta.
            $careerSql = "
                WITH dedup AS (
                    SELECT skiftraknare,
                           DATE(MIN(datum)) AS dag,
                ...
                per_op AS (
                    SELECT op1 AS op_num, skiftraknare, ibc_ok, drifttid_h
                    FROM dedup WHERE op1 > 0
                    UNION ALL
                    SELECT op2, skiftraknare, ibc_ok, drifttid_h
                    FROM dedup WHERE op2 > 0
                    UNION ALL
                    SELECT op3, skiftraknare, ibc_ok, drifttid_h
                    FROM dedup WHERE op3 > 0
                )
                SELECT
                    src.op_num,
                    ...
                    MAX(src.ibc_ok / NULLIF(src.drifttid_h, 0)) AS best_ibc_h
                FROM per_op src
            ...
            // ibc_ok is a PLC daily running counter — single LAG() query + PHP attribution.
            $monthFrom = date('Y-m-01', strtotime('-11 months'));

            $monthlySql = "
                SELECT skiftraknare, month_str, op1, op2, op3, drifttid, ibc_ok
                FROM (
                    SELECT skiftraknare, month_str, op1, op2, op3, drifttid,
                           ibc_ok
                    FROM (
                        SELECT skiftraknare,
                               DATE(MIN(datum))                 AS dag,
                               DATE_FORMAT(MIN(datum), '%Y-%m') AS month_str,
                               MAX(op1)      AS op1,
                               MAX(op2)      AS op2,
                               MAX(op3)      AS op3,
                               MAX(drifttid) AS drifttid,
                               MAX(ibc_ok)   AS ibc_ok
                        FROM rebotling_skiftrapport
                        WHERE datum >= :mfrom AND drifttid >= 30 AND ibc_ok > 0
                          AND (op1 > 0 OR op2 > 0 OR op3 > 0)
                        GROUP BY skiftraknare
                    ) dedup
                ) lagged
                WHERE ibc_ok > 0
            ...
            $monthlyStmt->execute([':mfrom' => $monthFrom]);
            $monthlyRows = $monthlyStmt->fetchAll(\PDO::FETCH_ASSOC);

            // PHP-side attribution to op1/op2/op3
            $monOpData = []; // [mon][op_num] => [ibc, hours, shifts]
            foreach ($monthlyRows as $r) {
                $ibc = (float)$r['ibc_ok'];
                $h   = (float)$r['drifttid'] / 60.0;
                $mon = $r['month_str'];
                foreach ([1, 2, 3] as $pos) {
                    $op = (int)$r["op{$pos}"];
                    if ($op <= 0) continue;
                    if (!isset($monOpData[$mon][$op])) {
                        $monOpData[$mon][$op] = ['ibc' => 0.0, 'hours' => 0.0, 'shifts' => 0];
                    }
                    $monOpData[$mon][$op]['ibc']    += $ibc;
                    $monOpData[$mon][$op]['hours']  += $h;
                    $monOpData[$mon][$op]['shifts']++;
                }
            }

            // Fetch operator names
            $opNameStmt = $pdo->query("SELECT number, name FROM operators");
            $opNames = [];
            foreach ($opNameStmt->fetchAll(\PDO::FETCH_ASSOC) as $r) {
                $opNames[(int)$r['number']] = $r['name'];
            }

            // Build monthlyRaw compatible with downstream code
            $monthlyRaw = [];
            foreach ($monOpData as $mon => $ops) {
                foreach ($ops as $opNum => $d) {
                    if ($d['shifts'] < 3) continue;
                    $ibcH = $d['hours'] > 0 ? $d['ibc'] / $d['hours'] : 0;
                    $monthlyRaw[] = [
                        'month_str'    => $mon,
                        'op_num'       => $opNum,
                        'op_name'      => $opNames[$opNum] ?? "#{$opNum}",
                        'month_ibc'    => (int)round($d['ibc']),
                        'month_hours'  => $d['hours'],
                        'month_ibc_h'  => $ibcH,
                        'month_shifts' => $d['shifts'],
                    ];
                }
            }
            // Sort: month ASC, ibc_h DESC
            usort($monthlyRaw, function($a, $b) {
                $mc = strcmp($a['month_str'], $b['month_str']);
                return $mc !== 0 ? $mc : $b['month_ibc_h'] <=> $a['month_ibc_h'];
            });
    ...
    private function getSkiftlagHistorik(): void {
        try {
            ...
            // ibc_ok is a PLC daily running counter — LAG() computes per-shift delta.
            // Canonically sort the 3 operators so team {3,5,7} ≡ {5,3,7}.
            $sql = "
                SELECT
                    lagged.min_op, lagged.mid_op, lagged.max_op,
                    SUM(lagged.ibc_ok) / NULLIF(SUM(lagged.drifttid / 60.0), 0) AS avg_ibc_h,
                    SUM(lagged.ibc_ok)            AS total_ibc,
                    SUM(lagged.drifttid / 60.0)   AS total_hours,
                    COUNT(*)                      AS total_shifts,
                    MIN(lagged.datum)             AS first_shift,
                    MAX(lagged.datum)             AS last_shift
                FROM (
                    SELECT
                        skiftraknare, datum, op1, op2, op3, drifttid,
                        ibc_ok,
                        LEAST(op1, op2, op3) AS min_op,
                        CASE
                            WHEN op1 NOT IN (LEAST(op1,op2,op3), GREATEST(op1,op2,op3)) THEN op1
                            WHEN op2 NOT IN (LEAST(op1,op2,op3), GREATEST(op1,op2,op3)) THEN op2
                            ELSE op3
                        END AS mid_op,
                        GREATEST(op1, op2, op3) AS max_op
                    FROM (
                        SELECT skiftraknare,
                               DATE(MAX(datum)) AS datum,
                               MAX(op1) AS op1, MAX(op2) AS op2, MAX(op3) AS op3,
                ...
                WHERE lagged.ibc_ok > 0
                GROUP BY lagged.min_op, lagged.mid_op, lagged.max_op
    ...
    private function getMomentum(): void {
        try {
            ...
            // Single query returns all shifts; PHP attributes to op1/op2/op3.
            $stmt = $this->pdo->prepare("
                SELECT skiftraknare, datum, op1, op2, op3, drifttid,
                       ibc_ok
                FROM (
                    SELECT skiftraknare,
                           DATE(MAX(datum)) AS datum,
            ...
            // Compute team avg IBC/h (SUM of per-shift deltas / SUM of runtime)
            $teamTotalIbc = 0;
            $teamTotalMin = 0;
            foreach ($rows as $r) {
                $teamTotalIbc += max(0, (int)$r['ibc_ok']);
                $teamTotalMin += max(0, (int)$r['drifttid']);
            }
            $teamAvg = $teamTotalMin > 0 ? $teamTotalIbc / ($teamTotalMin / 60.0) : 0.0;
            ...
            // Group shifts by operator (PHP attribution to op1/op2/op3)
            $byOp = [];
            foreach ($rows as $r) {
    ...
    private function getOperatorKonsistens(): void
    {
        try {
            ...
            // Compute team avg IBC/h from per-shift deltas
            $teamTotalIbc = 0;
            $teamTotalMin = 0;
            foreach ($rows as $r) {
                $teamTotalIbc += max(0, (int)$r['ibc_ok']);
            ...
            $teamAvgIbch = $teamTotalMin > 0
                ? $teamTotalIbc / ($teamTotalMin / 60.0)
                : 0.0;

            // Collect per-shift IBC/h values per operator (PHP attribution)
            $opVals = [];
            foreach ($rows as $r) {
                $ibc = max(0, (int)$r['ibc_ok']);
                $dt  = (float)$r['drifttid'];
                ...
                foreach (['op1', 'op2', 'op3'] as $pos) {
                    $opNum = (int)($r[$pos] ?? 0);
                    if ($opNum <= 0 || !isset($opNames[$opNum])) continue;
                    $opVals[$opNum][] = $ibcH;
                }
            }
    ...
    private function getProduktAnalys(): void {
        try {
            ...
            // Deduplicate by skiftraknare with LAG() for PLC running counters,
            // then aggregate per product
            $stmt = $this->pdo->prepare("
                SELECT
                    d.product_id,
                ...
                FROM (
                    SELECT product_id, drifttid, driftstopptime,
                           ibc_ok,
                           ibc_ej_ok,
                           bur_ej_ok
                    FROM (
            ...
            $tStmt = $this->pdo->prepare("
                SELECT
                    ...
                    SUM(d.ibc_ok) / NULLIF(SUM(d.drifttid) / 60.0, 0) AS ibc_per_h
                FROM (
                    SELECT product_id, datum, drifttid,
                           ibc_ok
                    FROM (
                        SELECT skiftraknare,
                               DATE(MAX(datum))   AS datum,
    ...
    private function getVeckansTopplista(): void
    {
        try {
            ...
            // ibc_ok is a PLC daily running counter — LAG() computes per-shift delta.
            // One row per unique shift with correct per-shift ibc_ok.
            $stmt = $this->pdo->prepare("
                SELECT yw, week_start, op1, op2, op3, drifttid,
                       ibc_ok
                FROM (
                    SELECT skiftraknare,
                           DATE(MAX(datum))         AS dag,
                           YEARWEEK(MAX(datum), 1)  AS yw,
                           MAX(datum)               AS week_start,
                           MAX(op1) AS op1, MAX(op2) AS op2, MAX(op3) AS op3,
                           MAX(drifttid)             AS drifttid,
                           MAX(ibc_ok)               AS ibc_ok
                    FROM rebotling_skiftrapport
    ...
    private function getOperatorAvsaknad(): void {
        try {
            ...
            // ibc_ok is a PLC daily running counter — LAG() computes per-shift delta so
            // multi-shift days are not double-counted.
            $stmt = $this->pdo->prepare("
                SELECT op1, op2, op3, drifttid,
                       ibc_ok
                FROM (
    ...
    private function getKassationsKarta(): void {
        try {
            ...
            // deltas. Single query; PHP attributes to op1/op2/op3.
            $stmt = $pdo->prepare("
                SELECT skiftraknare, datum, op1, op2, op3, product_id,
                       ibc_ok,
                       ibc_ej_ok
            ...
            // Aggregate per (op_num × product_id) with PHP attribution
            $cellsAgg  = [];
            $cells    = [];
            $usedOps  = [];
            $usedProds = [];
            ...
            foreach ($rows as $r) {
                $ibc_ok = max(0, (int)$r['ibc_ok']);
                $ibc_ej = max(0, (int)$r['ibc_ej_ok']);
                $total  = $ibc_ok + $ibc_ej;
                $prodId = (int)$r['product_id'];
                if ($prodId <= 0 || !isset($prodMap[$prodId])) continue;
                foreach (['op1', 'op2', 'op3'] as $pos) {
                    $opNum = (int)($r[$pos] ?? 0);
                    if ($opNum <= 0 || !isset($opMap[$opNum])) continue;
                    if (!isset($cellsAgg[$opNum][$prodId])) {
                        $cellsAgg[$opNum][$prodId] = ['ej' => 0, 'total' => 0, 'shifts' => 0];
                    }
                    $cellsAgg[$opNum][$prodId]['ej']     += $ibc_ej;
                    $cellsAgg[$opNum][$prodId]['total']  += $total;
                    $cellsAgg[$opNum][$prodId]['shifts'] += 1;
                }
            }

            foreach ($cellsAgg as $opNum => $prodData) {
                foreach ($prodData as $prodId => $agg) {
                    if ($agg['total'] < 5) continue;
                    $kassation = $agg['total'] > 0 ? round($agg['ej'] / $agg['total'] * 100, 2) : 0.0;
                    $cells[] = [
                        'op_num'        => $opNum,
                        'product_id'    => $prodId,
                        'kassation_pct' => $kassation,
                        'skift_count'   => $agg['shifts'],
                        'total_ibc'     => $agg['total'],
                        'total_ibc_ej'  => $agg['ej'],
                    ];
    ...
    private function getKassationTrend(): void {
        try {
            ...
            // ibc_ok/ibc_ej_ok are PLC daily running counters — LAG() computes per-shift
            // deltas. Single query; PHP attributes to op1/op2/op3 per week.
            $stmt = $this->pdo->prepare("
                SELECT datum, YEARWEEK(datum, 1) AS yw, op1, op2, op3,
                       ibc_ok,
                       ibc_ej_ok
                FROM (
                    SELECT skiftraknare,
                    ...
                    FROM rebotling_skiftrapport
                    WHERE datum BETWEEN :from AND :to
                      AND (op1 IS NOT NULL OR op2 IS NOT NULL OR op3 IS NOT NULL)
                    GROUP BY skiftraknare
                ) raw
                ORDER BY datum, skiftraknare
            ");
            ...
            // Team-level per-week (aggregate all ops) + per-op attribution
            $teamByYw = [];
            ...
            foreach ($rawRows as $r) {
                ...
                $teamByYw[$yw]['ej']    += $ibc_ej;
                $teamByYw[$yw]['total'] += $total;

                foreach (['op1', 'op2', 'op3'] as $pos) {
                    $opNum = (int)($r[$pos] ?? 0);
                    if ($opNum <= 0 || !isset($opMap[$opNum])) continue;
                    if (!isset($opData[$opNum][$yw])) {
                        $opData[$opNum][$yw] = ['total_ej' => 0, 'total_ibc' => 0, 'skifter' => 0];
                    }
                    $opData[$opNum][$yw]['total_ej']  += $ibc_ej;
                    $opData[$opNum][$yw]['total_ibc'] += $total;
                    $opData[$opNum][$yw]['skifter']   += 1;
                }
            }
    ...
    private function getCoachView(): void {
        try {
            ...
            // ibc_ok/ibc_ej_ok are PLC daily running counters — LAG() computes per-shift
            // deltas so multi-shift days are not over-credited.
            $stmt = $this->pdo->prepare("
                SELECT op1, op2, op3, drifttid, datum,
                       ibc_ok,
                       ibc_ej_ok
            ...
            // Team IBC/h over 90d (SUM of per-shift deltas)
            $teamIbc = 0;
            $teamMin = 0;
            foreach ($allShifts as $s) {
    ...
    private function getProduktionsTidsserie(): void {
        try {
            ...
            // Monthly aggregation — same LAG() pattern
            $sqlMonthly = "
                SELECT
    ...
    private function getRastAnalys(): void {
        try {
            ...
            // ibc_ok is a PLC daily running counter — LAG() gives per-shift delta.
            // drifttid/rasttime/driftstopptime reset per shift so MAX() is correct for those.
            $stmt = $this->pdo->prepare("
                SELECT skiftraknare, datum, drifttid, rasttime, driftstopptime, ibc_ok
                FROM (
                    SELECT skiftraknare, datum, drifttid, rasttime, driftstopptime,
                           ibc_ok
                    FROM (
                        SELECT
                            skiftraknare,
    ...
    private function getSasongsanalys(): void {
        try {
            // Per year+month — LAG() per-shift ibc_ok delta (PLC counter resets at midnight)
            $sqlYearMonth = "
                SELECT
                    YEAR(datum)  AS ar,
                    MONTH(datum) AS man,
        ...
        try {
            ...
            $sql = "
                SELECT dow, skift_typ,
                       SUM(tot_ibc)    AS tot_ibc,
                       SUM(tot_min)    AS tot_min,
                       COUNT(*)        AS antal_skift
                FROM (
    ...
    private function getSpeedQualityCorrelation(): void {
        ...
        try {
            $sql = "
                ...
                FROM (
                    SELECT skiftraknare, datum, op1, op2, op3, product_id, drifttid, start_hour,
                           ibc_ok,
                           ibc_ej_ok
                    FROM (
                        SELECT skiftraknare,
    ...
    private function getKassationsrytm(): void {
        try {
            ...
            $sql = "
                SELECT DAYOFWEEK(datum) AS dow, skift_typ,
                       SUM(ibc_ok)    AS tot_ibc_ok,
                       SUM(ibc_ej_ok) AS tot_ibc_ej,
                       SUM(bur_ej_ok) AS tot_bur_ej,
                       COUNT(*)       AS antal_skift
                FROM (
                    SELECT skiftraknare,
                           DATE(MAX(datum))              AS datum,
        ...
        try {
            ...
            $sql = "
                ...
                SELECT u.op_num, u.pos,
                       ...
                       COUNT(*)     AS skift_count
                FROM (
                    SELECT op1 AS op_num, 'op1' AS pos, ibc_ok AS ibc, drifttid AS min
                    FROM dedup WHERE op1 IS NOT NULL AND op1 > 0
                    UNION ALL
                    SELECT op2, 'op2', ibc_ok, drifttid FROM dedup WHERE op2 IS NOT NULL AND op2 > 0
                    UNION ALL
                    SELECT op3, 'op3', ibc_ok, drifttid FROM dedup WHERE op3 IS NOT NULL AND op3 > 0
                ) u
    ...
    private function getVaderProduktion(): void {
        ...
        $sql = "
            WITH daily_prod AS (
                ...
                FROM (
                    SELECT dag, skiftraknare, drifttid,
                           ibc_ok
                    FROM (
                        SELECT skiftraknare,
                               DATE(MAX(datum))              AS dag,
    ...
    private function getPersonalKalender(): void {
        ...
        try {
            // 1. Daily IBC/h: LAG() per-shift delta, then sum per day
            $sql = "
                SELECT datum,
    ...
    private function getSkiftSekvens(): void {
        ...
        try {
            // ibc_ok is a PLC daily running counter — compute LAG delta before UNION ALL attribution.
            $sql = "
                WITH lag_base AS (
                    SELECT skiftraknare,
                           DATE(MAX(datum))              AS datum,
                ...
                base_shifts AS (
                    SELECT op1 AS op_num, datum, skiftraknare, ibc_ok, drifttid
                    FROM lag_shifts WHERE op1 IS NOT NULL AND op1 > 0
                    UNION ALL
                    SELECT op2, datum, skiftraknare, ibc_ok, drifttid
                    FROM lag_shifts WHERE op2 IS NOT NULL AND op2 > 0
                    UNION ALL
                    SELECT op3, datum, skiftraknare, ibc_ok, drifttid
                    FROM lag_shifts WHERE op3 IS NOT NULL AND op3 > 0
                ),
                with_lag AS (
    ...
    private function getProduktionsmaal(): void {
        try {
            ...
            // Recent 14-day pace for projection — LAG() per-shift delta
    ...
    private function getOperatorUtveckling(): void {
        try {
            ...
            // ibc_ok is a PLC daily running counter — LAG() computes per-shift delta.
            $stmt = $this->pdo->prepare("
                SELECT DATE_FORMAT(datum, '%Y-%m') AS ym, op1, op2, op3, drifttid,
                       ibc_ok
                FROM (
                    SELECT skiftraknare,
                           DATE(MAX(datum)) AS datum,
    ...
    private function getVeckosammanfattning(): void {
        ...
        try {
            ...
            // ibc_ok is a daily PLC running counter — MAX gives the day total (final counter).
            $sqlDay = "
                SELECT
                    datum,
                    DAYOFWEEK(datum) AS dow,
            ...
            // LAG() gives correct per-shift delta for the daily running counter.
            $sqlOp = "
                WITH dedup AS (
                    SELECT skiftraknare,
                           DATE(MIN(datum)) AS dag,
                ...
                per_op AS (
                    SELECT op1 AS op_num, skiftraknare, ibc_ok, drifttid_min
                    FROM dedup WHERE op1 IS NOT NULL AND op1 > 0
                    UNION ALL
                    SELECT op2, skiftraknare, ibc_ok, drifttid_min
                    FROM dedup WHERE op2 IS NOT NULL AND op2 > 0
                    UNION ALL
                    SELECT op3, skiftraknare, ibc_ok, drifttid_min
                    FROM dedup WHERE op3 IS NOT NULL AND op3 > 0
                )
                SELECT
                    src.op_num,
                    COALESCE(o.name, CONCAT('Op ', src.op_num)) AS op_name,
                    SUM(src.ibc_ok)       AS ibc_ok,
                    SUM(src.drifttid_min) AS drifttid_min,
                    COUNT(*)              AS antal_skift
                FROM per_op src
                LEFT JOIN operators o ON o.number = src.op_num
                GROUP BY src.op_num, op_name
                HAVING drifttid_min > 0
                ORDER BY (SUM(src.ibc_ok) / (SUM(src.drifttid_min) / 60)) DESC
            ";
            ...
            // Two-level aggregation: MAX per day gives correct daily total (counter resets each day).
            $sqlKpi = "
                SELECT
                    SUM(day_ibc)      AS ibc_ok,
                    SUM(drifttid_min) AS drifttid_min,
                    SUM(stopp_min)    AS stopp_min,
                    SUM(day_tot)      AS tot_ibc,
                    SUM(skift_count)  AS antal_skift
                FROM (
                    SELECT datum,
                           MAX(ibc_ok_s)    AS day_ibc,
                           SUM(drifttid_s)  AS drifttid_min,
                           SUM(stopp_s)     AS stopp_min,
                           MAX(tot_s)       AS day_tot,
                           COUNT(*)         AS skift_count
                    FROM (
                        SELECT datum, skiftraknare,
                               MAX(ibc_ok)                                              AS ibc_ok_s,
                               MAX(drifttid)                                            AS drifttid_s,
                               MAX(COALESCE(driftstopptime,0))                          AS stopp_s,
                               MAX(ibc_ok + COALESCE(ibc_ej_ok,0) + COALESCE(bur_ej_ok,0)) AS tot_s
                        FROM rebotling_skiftrapport
                        WHERE datum BETWEEN :from3 AND :to3
                          AND drifttid >= 30
                        GROUP BY skiftraknare, datum
    ...
    private function getOEEDashboard(): void {
        ...
        try {
            ...
            // ibc_ok/ibc_ej_ok/totalt are PLC daily running counters — LAG() gives per-shift
            // delta so later shifts in a multi-shift day don't inherit the cumulative total.
            // drifttid and driftstopptime reset per shift so MAX() is correct for those.
            $sql = "
                SELECT
                    d.skiftraknare,
                    d.datum,
                    ...
                    COALESCE(p.cycle_time_minutes, 0)                  AS cycle_time_minutes
                FROM (
                    SELECT skiftraknare, datum, drifttid, driftstopptime, product_id,
                           ibc_ok,
                           ibc_ej_ok,
                           totalt
                    FROM (
    ...
    private function getRotationsanalys(): void {
        try {
            ...
            // ibc_ok is a daily PLC running counter — dedup per skiftraknare + LAG() gives
            // correct per-shift delta before attributing to each operator position.
            $sql = "
                WITH dedup AS (
                    SELECT skiftraknare,
                           DATE(MIN(datum)) AS dag,
    ...
    private function getKassationstyper(): void {
        try {
            ...
            // ibc_ok/ibc_ej_ok/bur_ej_ok are PLC daily running counters — use per-day MAX
            // (counters reset at midnight) for facility-level monthly and product aggregations.
            $sqlMonthly = "
                SELECT
                    DATE_FORMAT(dag, '%Y-%m') AS month,
                    SUM(day_ibc_ok)    AS ibc_ok,
                    SUM(day_ibc_ej_ok) AS ibc_ej_ok,
                    SUM(day_bur_ej_ok) AS bur_ej_ok,
                    SUM(day_totalt)    AS totalt
                FROM (
                    SELECT DATE(datum) AS dag,
                           MAX(COALESCE(ibc_ok,    0)) AS day_ibc_ok,
                           MAX(COALESCE(ibc_ej_ok, 0)) AS day_ibc_ej_ok,
                           MAX(COALESCE(bur_ej_ok, 0)) AS day_bur_ej_ok,
                           MAX(COALESCE(totalt,    0)) AS day_totalt
                    FROM rebotling_skiftrapport
                    WHERE datum BETWEEN :from1 AND :to1
                    GROUP BY DATE(datum)
                ) days
                GROUP BY DATE_FORMAT(dag, '%Y-%m')
                ORDER BY month ASC
            ";
            $stmtM = $this->pdo->prepare($sqlMonthly);
            ...
            // Per-product breakdown — LAG() per-shift delta, then group by product_id
            $sqlProd = "
                SELECT
            ...
            // Per-operator breakdown — LAG() per-shift delta + PHP op1/op2/op3 attribution
            $stmtOps = $this->pdo->query("SELECT number, name FROM operators WHERE active=1 ORDER BY name");
            $opNames = [];
            ...
            $stmtO->execute([':from3' => $from, ':to3' => $to]);
            $opRows = $stmtO->fetchAll(\PDO::FETCH_ASSOC);

            $byOpAgg = [];
            foreach ($opRows as $r) {
                $ibc   = max(0, (int)$r['ibc_ok']);
                $ibcEj = max(0, (int)$r['ibc_ej_ok']);
                $burEj = max(0, (int)$r['bur_ej_ok']);
                foreach (['op1', 'op2', 'op3'] as $pos) {
                    $opNum = (int)($r[$pos] ?? 0);
                    if ($opNum <= 0 || !isset($opNames[$opNum])) continue;
                    if (!isset($byOpAgg[$opNum])) {
                        $byOpAgg[$opNum] = ['ibc_ok' => 0, 'ibc_ej_ok' => 0, 'bur_ej_ok' => 0, 'skift_count' => 0];
                    }
                    $byOpAgg[$opNum]['ibc_ok']    += $ibc;
                    $byOpAgg[$opNum]['ibc_ej_ok'] += $ibcEj;
                    $byOpAgg[$opNum]['bur_ej_ok'] += $burEj;
                    $byOpAgg[$opNum]['skift_count']++;
                }
            }

            $byOperator = [];
            foreach ($byOpAgg as $opNum => $agg) {
                if ($agg['skift_count'] < 3) continue;
                $ibcTotal = $agg['ibc_ok'] + $agg['ibc_ej_ok'];
                $allTotal = $ibcTotal + $agg['bur_ej_ok'];
                $byOperator[] = [
                    'number'       => $opNum,
                    'name'         => $opNames[$opNum] ?? "Op {$opNum}",
                    'ibc_ok'       => $agg['ibc_ok'],
                    ...
                    'ibc_kass_pct' => $ibcTotal > 0
                        ? round($agg['ibc_ej_ok'] / $ibcTotal * 100, 2) : 0.0,
                    'bur_kass_pct' => $allTotal > 0
                        ? round($agg['bur_ej_ok'] / $allTotal * 100, 2) : 0.0,
                ];
            }
            usort($byOperator, fn($a, $b) => ($b['ibc_ej_ok'] + $b['bur_ej_ok']) <=> ($a['ibc_ej_ok'] + $a['bur_ej_ok']));
    ...
    private function getPositionsSpecialisering(): void {
        try {
            ...
            // ibc_ok is a PLC daily running counter — LAG() computes per-shift delta.
            // Single query; PHP attributes to op1/op2/op3 with position label.
            $stmt = $this->pdo->prepare("
                SELECT datum, op1, op2, op3, drifttid,
                       ibc_ok
                FROM (
            ...
            // Group by operator: accumulate ibc and hours per position (op1/op2/op3)
            $byOpAgg = []; // [opNum][pos] => ['ibc'=>0,'hours'=>0,'shifts'=>0,'name'=>'']
            $opNames2 = [];
            $opNameStmt = $this->pdo->query("SELECT number, name FROM operators WHERE active=1");
            foreach ($opNameStmt->fetchAll(\PDO::FETCH_ASSOC) as $op) {
                $opNames2[(int)$op['number']] = $op['name'];
            }

            foreach ($rows as $r) {
                $ibc = max(0, (float)$r['ibc_ok']);
                $hrs = (float)$r['drifttid'] / 60.0;
                if ($hrs <= 0) continue;
                $positions = ['op1' => (int)$r['op1'], 'op2' => (int)$r['op2'], 'op3' => (int)$r['op3']];
                foreach ($positions as $pos => $opNum) {
                    if ($opNum <= 0) continue;
                    $name = $opNames2[$opNum] ?? "Op $opNum";
                    if (!isset($byOpAgg[$opNum])) {
                        $byOpAgg[$opNum] = ['name' => $name, 'pos' => []];
                    }
                    if (!isset($byOpAgg[$opNum]['pos'][$pos])) {
                        $byOpAgg[$opNum]['pos'][$pos] = ['ibc' => 0.0, 'hours' => 0.0, 'shifts' => 0];
                    }
                    $byOpAgg[$opNum]['pos'][$pos]['ibc']    += $ibc;
                    $byOpAgg[$opNum]['pos'][$pos]['hours']  += $hrs;
                    $byOpAgg[$opNum]['pos'][$pos]['shifts'] += 1;
                }
            }

            // Convert to rows format expected by downstream code
            $rawRowsByPos = [];
            foreach ($byOpAgg as $opNum => $data) {
                foreach ($data['pos'] as $pos => $agg) {
                    $rawRowsByPos[] = [
                        'op_num'      => $opNum,
                        'pos'         => $pos,
                        'op_name'     => $data['name'],
                        'antal_skift' => $agg['shifts'],
                        'total_ibc'   => $agg['ibc'],
                        'total_hours' => $agg['hours'],
                    ];
                }
            }
            // Sort: by op_num, then pos
            usort($rawRowsByPos, fn($a, $b) => $a['op_num'] <=> $b['op_num'] ?: strcmp($a['pos'], $b['pos']));

            // Group by operator (preserve the original variable name $byOp for downstream code)
            $byOp = [];
            foreach ($rawRowsByPos as $r) {
                $opNum = (int)$r['op_num'];
                if (!isset($byOp[$opNum])) {
                    $byOp[$opNum] = [
                        'number' => $opNum,
    ...
    private function getProduktKvalitetstrender(): void {
        try {
            ...
            // LAG() per-shift ibc_ok/ibc_ej_ok/bur_ej_ok deltas, then aggregate by month + product
            $sql = "
                SELECT s.yr, s.mo, s.product_id,
                       MAX(COALESCE(p.name, CONCAT('Produkt ', s.product_id))) AS product_name,
                       SUM(s.ibc_ej_ok)  AS total_ibc_ej,
                       SUM(s.bur_ej_ok)  AS total_bur_ej,
                       SUM(s.totalt)     AS total_tot,
                       SUM(s.ibc_ok)     AS total_ibc_ok,
                       COUNT(*)          AS num_shifts
                FROM (
                    SELECT skiftraknare,
                           YEAR(MAX(datum))                AS yr,
                           MONTH(MAX(datum))               AS mo,
                           MAX(product_id)                 AS product_id,
                           MAX(COALESCE(ibc_ok,    0))     AS ibc_ok,
                           MAX(COALESCE(ibc_ej_ok, 0))     AS ibc_ej_ok,
                           MAX(COALESCE(bur_ej_ok, 0))     AS bur_ej_ok,
    ...
    private function getMilstolpar(): void {
        try {
            ...
            $careerSql = "
                ...
                per_op AS (
                    SELECT op1 AS op_num, skiftraknare, datum, ibc_ok, drifttid_h
                    FROM dedup WHERE op1 IS NOT NULL AND op1 > 0
                    UNION ALL
                    SELECT op2, skiftraknare, datum, ibc_ok, drifttid_h
                    FROM dedup WHERE op2 IS NOT NULL AND op2 > 0
                    UNION ALL
                    SELECT op3, skiftraknare, datum, ibc_ok, drifttid_h
                    FROM dedup WHERE op3 IS NOT NULL AND op3 > 0
                )
                SELECT
                    src.op_num,
                    ...
                    CASE WHEN SUM(src.drifttid_h) > 0
                         THEN SUM(src.ibc_ok) / SUM(src.drifttid_h)
                         ELSE 0 END                             AS career_ibc_h,
                    MAX(src.ibc_ok / NULLIF(src.drifttid_h, 0)) AS best_shift_ibc_h,
            ...
            $monthSql = "
                ...
                per_op AS (
                    SELECT op1 AS op_num, skiftraknare, datum, ibc_ok
                    FROM dedup WHERE op1 IS NOT NULL AND op1 > 0
                    UNION ALL
                    SELECT op2, skiftraknare, datum, ibc_ok
                    FROM dedup WHERE op2 IS NOT NULL AND op2 > 0
                    UNION ALL
                    SELECT op3, skiftraknare, datum, ibc_ok
                    FROM dedup WHERE op3 IS NOT NULL AND op3 > 0
                )
                SELECT
                    src.op_num,
    ...
    private function getArsKurva(): void {
        try {
            // LAG() per-shift ibc_ok delta, then aggregate per year+dayofyear
            $stmt = $this->pdo->prepare("
                SELECT
                    YEAR(datum)      AS ar,
    ...
    private function getSchemaRekommendationer(): void {
        try {
            ...
            // --- Query 1: per-operator per-position IBC/h — CTE LAG() per-shift delta ---
            $stmt1 = $this->pdo->prepare("
                ...
                FROM (
                    SELECT op1 AS op_num, 'op1' AS pos, skiftraknare, ibc_ok AS ibc, drifttid/60.0 AS hours
                    FROM lag_shifts WHERE op1 > 0
                    UNION ALL
                    SELECT op2, 'op2', skiftraknare, ibc_ok, drifttid/60.0
                    FROM lag_shifts WHERE op2 > 0
                    UNION ALL
                    SELECT op3, 'op3', skiftraknare, ibc_ok, drifttid/60.0
                    FROM lag_shifts WHERE op3 > 0
                ) u
                LEFT JOIN operators o ON o.number = u.op_num
            ...
            // --- Query 2: per-operator per-shift-type IBC/h — CTE LAG() per-shift delta ---
            $stmt2 = $this->pdo->prepare("
                WITH lag_shifts AS (
                    SELECT skiftraknare, op1, op2, op3, drifttid, skift_typ,
                           ibc_ok
                    FROM (
                        SELECT skiftraknare,
                               DATE(MAX(datum)) AS datum,
                ...
                FROM (
                    SELECT op1 AS op_num, skiftraknare, skift_typ, ibc_ok AS ibc, drifttid/60.0 AS hours
                    FROM lag_shifts WHERE op1 > 0
                    UNION ALL
                    SELECT op2, skiftraknare, skift_typ, ibc_ok, drifttid/60.0
                    FROM lag_shifts WHERE op2 > 0
                    UNION ALL
                    SELECT op3, skiftraknare, skift_typ, ibc_ok, drifttid/60.0
                    FROM lag_shifts WHERE op3 > 0
                ) u
            ...
            // --- Query 3: per-operator overall + 30d-window trend — CTE LAG() per-shift delta ---
            $stmt3 = $this->pdo->prepare("
                ...
                FROM (
                    SELECT op1 AS op_num, datum, skiftraknare, ibc_ok AS ibc, drifttid/60.0 AS hours
                    FROM lag_shifts WHERE op1 > 0
                    UNION ALL
                    SELECT op2, datum, skiftraknare, ibc_ok, drifttid/60.0
                    FROM lag_shifts WHERE op2 > 0
                    UNION ALL
                    SELECT op3, datum, skiftraknare, ibc_ok, drifttid/60.0
                    FROM lag_shifts WHERE op3 > 0
                ) u
                GROUP BY op_num
            ");
    ...
    private function getIdag(): void {
        ...
        try {
            ...
            // ibc_ok/ibc_ej_ok/bur_ej_ok/totalt reset per skiftraknare —
            // MAX() per skiftraknare gives the correct per-shift total. No LAG needed.
            $stmt = $this->pdo->prepare("
                SELECT
                    r.skiftraknare,
                    MIN(r.datum)          AS datum,
                    MAX(r.ibc_ok)         AS ibc_ok,
                    MAX(r.ibc_ej_ok)      AS ibc_ej_ok,
                    MAX(r.bur_ej_ok)      AS bur_ej_ok,
                    MAX(r.totalt)         AS totalt,
    ...
    // GET ?action=rebotling&run=produkt-jamforelse&days=90[&prodA=1&prodB=2]
    // Returns product list (always) + comparison data when prodA+prodB are given.
    private function getProduktJamforelse(): void {
        try {
            $days  = max(30, min(730, (int)($_GET['days'] ?? 90)));
            $to    = date('Y-m-d');
            $from  = date('Y-m-d', strtotime("-{$days} days"));
            $t6from = date('Y-m-d', strtotime('-180 days'));
            $prodA = isset($_GET['prodA']) ? (int)$_GET['prodA'] : 0;
            $prodB = isset($_GET['prodB']) ? (int)$_GET['prodB'] : 0;

            // Always return product list for the dropdowns
            $listStmt = $this->pdo->prepare("
                SELECT d.product_id,
                       COALESCE(p.name, CONCAT('Produkt ', d.product_id)) AS name,
                       COUNT(*)       AS antal_skift,
                       SUM(d.ibc_ok) / NULLIF(SUM(d.drifttid) / 60.0, 0) AS ibc_per_h
                FROM (
                    SELECT product_id, drifttid,
                           ibc_ok
                    FROM (
                        SELECT skiftraknare,
                               DATE(MAX(datum))  AS datum,
                ...
                ORDER BY ibc_per_h DESC
            ");
            $listStmt->execute([':from1' => $from, ':to1' => $to]);
            $productList = [];
            ...
            if ($prodA <= 0 || $prodB <= 0 || $prodA === $prodB) {
                echo json_encode([
                    'success'    => true,
            ...
            // --- Main KPI query for both products in one go ---
            $kpiStmt = $this->pdo->prepare("
                SELECT d.product_id,
                       COALESCE(p.name, CONCAT('Produkt ', d.product_id)) AS name,
                       COALESCE(p.cycle_time_minutes, 0)                   AS cycle_time,
                       COUNT(*)          AS antal_skift,
                       SUM(d.ibc_ok)     AS total_ibc,
                       SUM(d.ibc_ej)     AS total_ibc_ej,
                       SUM(d.drifttid)   AS total_min,
                       SUM(d.stopptid)   AS total_stopp_min
                FROM (
                    SELECT product_id, drifttid, stopptid,
                           ibc_ok,
                           ibc_ej
                    FROM (
                        SELECT skiftraknare,
                               ...
                               MAX(ibc_ej_ok)      AS ibc_ej_ok,
                               MAX(drifttid)       AS drifttid,
                               MAX(driftstopptime) AS stopptid
                        FROM rebotling_skiftrapport
                        WHERE datum >= :kfrom AND datum <= :kto
                          AND drifttid >= 30
                          AND product_id IN (:kpa, :kpb)
                        GROUP BY skiftraknare
                    ) base
                ) d
                LEFT JOIN rebotling_products p ON p.id = d.product_id
                GROUP BY d.product_id, p.name, p.cycle_time_minutes
            ");
            $kpiStmt->execute([':kfrom' => $from, ':kto' => $to, ':kpa' => $prodA, ':kpb' => $prodB]);
            $kpiMap = [];
            foreach ($kpiStmt->fetchAll(\PDO::FETCH_ASSOC) as $r) {
                $kpiMap[(int)$r['product_id']] = $r;
            }

            // --- Monthly trend (6 months) for both products ---
            $trendStmt = $this->pdo->prepare("
                SELECT d.product_id,
                       ...
                       COUNT(*)       AS antal,
                       SUM(d.ibc_ok) / NULLIF(SUM(d.drifttid) / 60.0, 0) AS ibc_per_h,
                       CASE WHEN SUM(d.ibc_ok) + SUM(d.ibc_ej) > 0
                            THEN SUM(d.ibc_ej) * 100.0 / (SUM(d.ibc_ok) + SUM(d.ibc_ej))
                            ELSE 0 END AS kassgrad
                FROM (
                    SELECT product_id, datum, drifttid,
                           ibc_ok,
                    ...
                    FROM (
                        SELECT skiftraknare,
                               ...
                               MAX(drifttid)     AS drifttid
                        FROM rebotling_skiftrapport
                        WHERE datum >= :tfrom AND datum <= :tto
                          AND drifttid >= 30
                          AND product_id IN (:tpa, :tpb)
                        GROUP BY skiftraknare
                    ) base
                ) d
                GROUP BY d.product_id, yr, mo
                ORDER BY d.product_id, yr, mo
            ");
            $trendStmt->execute([':tfrom' => $t6from, ':tto' => $to, ':tpa' => $prodA, ':tpb' => $prodB]);
            $trendMap = [];
            foreach ($trendStmt->fetchAll(\PDO::FETCH_ASSOC) as $r) {
                $pid = (int)$r['product_id'];
                if (!isset($trendMap[$pid])) $trendMap[$pid] = [];
                $trendMap[$pid][] = [
                    'yr'       => (int)$r['yr'],
                    'mo'       => (int)$r['mo'],
                    'antal'    => (int)$r['antal'],
            ...
            // --- Top operators: who has the most shifts on each product (shared IBC/h = team metric) ---
            $opStmt = $this->pdo->prepare("
                SELECT d.product_id, d.op_num,
                       COALESCE(o.name, CONCAT('Op ', d.op_num)) AS op_name,
                       COUNT(DISTINCT d.skiftraknare) AS antal_skift
                FROM (
                    SELECT product_id, skiftraknare, op1 AS op_num
                    FROM rebotling_skiftrapport
                    WHERE datum >= :ofrom AND datum <= :oto
                      AND drifttid >= 30 AND op1 > 0
                      AND product_id IN (:opa, :opb)
                    UNION ALL
                    SELECT product_id, skiftraknare, op2
                    FROM rebotling_skiftrapport
                    WHERE datum >= :ofrom2 AND datum <= :oto2
                      AND drifttid >= 30 AND op2 > 0
                      AND product_id IN (:opa2, :opb2)
                    UNION ALL
                    SELECT product_id, skiftraknare, op3
                    FROM rebotling_skiftrapport
                    WHERE datum >= :ofrom3 AND datum <= :oto3
                      AND drifttid >= 30 AND op3 > 0
                      AND product_id IN (:opa3, :opb3)
                ) d
                LEFT JOIN operators o ON o.number = d.op_num
                GROUP BY d.product_id, d.op_num, o.name
                HAVING antal_skift >= 2
                ORDER BY d.product_id, antal_skift DESC
            ");
            $opStmt->execute([
                ':ofrom' => $from, ':oto' => $to, ':opa' => $prodA, ':opb' => $prodB,
                ':ofrom2' => $from, ':oto2' => $to, ':opa2' => $prodA, ':opb2' => $prodB,
                ':ofrom3' => $from, ':oto3' => $to, ':opa3' => $prodA, ':opb3' => $prodB,
            ]);
            $opMap = [];
            foreach ($opStmt->fetchAll(\PDO::FETCH_ASSOC) as $r) {
                $pid = (int)$r['product_id'];
                if (!isset($opMap[$pid])) $opMap[$pid] = [];
                if (count($opMap[$pid]) < 5) {
                    $opMap[$pid][] = [
                        'number'     => (int)$r['op_num'],
                        'name'       => $r['op_name'],
                        'antal_skift'=> (int)$r['antal_skift'],
            ...
            // --- Build structured data for each product ---
            $buildData = function (int $pid) use ($kpiMap, $trendMap, $opMap): array {
                $r          = $kpiMap[$pid] ?? [];
                $totalMin   = (float)($r['total_min']       ?? 0);
                $totalIbc   = (float)($r['total_ibc']       ?? 0);
                $totalIbcEj = (float)($r['total_ibc_ej']    ?? 0);
                $totalStopp = (float)($r['total_stopp_min'] ?? 0);
                $cycleTime  = (float)($r['cycle_time']      ?? 0);
                $antalSkift = (int)($r['antal_skift']        ?? 0);
                $ibcH       = $totalMin > 0 ? round($totalIbc / ($totalMin / 60.0), 2) : 0.0;
                $kassgrad   = ($totalIbc + $totalIbcEj) > 0
                    ? round($totalIbcEj * 100.0 / ($totalIbc + $totalIbcEj), 2) : 0.0;
                $stoppgrad  = ($totalMin + $totalStopp) > 0
                    ? round($totalStopp * 100.0 / ($totalMin + $totalStopp), 2) : 0.0;
                $expectedH  = $cycleTime > 0 ? round(60.0 / $cycleTime, 2) : null;
                $effPct     = ($expectedH !== null && $expectedH > 0)
                    ? round($ibcH / $expectedH * 100.0, 1) : null;
                return [
                    'id'            => $pid,
                    'name'          => $r['name'] ?? "Produkt $pid",
                    'ibc_per_h'     => $ibcH,
                    'kassgrad'      => $kassgrad,
                    'stoppgrad'     => $stoppgrad,
                    'antal_skift'   => $antalSkift,
                    'total_ibc'     => (int)$totalIbc,
                    'total_timmar'  => round($totalMin / 60.0, 1),
                    'cycle_time'    => $cycleTime,
                    'expected_ibch' => $expectedH,
                    'effektivitet'  => $effPct,
                    'trend'         => $trendMap[$pid] ?? [],
                    'top_ops'       => $opMap[$pid] ?? [],
                ];
            ...
            echo json_encode([
                ...
                'to'         => $to,
                'products'   => $productList,
                'comparison' => ['a' => $buildData($prodA), 'b' => $buildData($prodB)],
            ], \JSON_UNESCAPED_UNICODE);
        ...
        try {
            ...
            $sqlDaily = "
                SELECT dag,
                       ...
                       SUM(ibc_ej_ok + bur_ej_ok)   AS kassation,
            ...
            $sqlProd = "
                SELECT rs.product_id,
                       ...
                       SUM(rs.ibc_ej_ok + rs.bur_ej_ok) AS kassation,
                       SUM(rs.ibc_ej_ok)                 AS ibc_ej,
                       SUM(rs.bur_ej_ok)                 AS bur_ej,
                       SUM(rs.totalt)                    AS totalt
                FROM (
                ...
                HAVING SUM(rs.totalt) > 0
                ORDER BY (SUM(rs.ibc_ej_ok + rs.bur_ej_ok)) / SUM(rs.totalt) DESC
                LIMIT 10
            ";
            ...
            $sqlOp = "
                ...
                all_ops AS (
                    SELECT op1 AS op_num, ibc_ej_ok, bur_ej_ok, totalt, skiftraknare FROM per_shift WHERE op1 IS NOT NULL AND op1 > 0
                    UNION ALL
                    SELECT op2, ibc_ej_ok, bur_ej_ok, totalt, skiftraknare FROM per_shift WHERE op2 IS NOT NULL AND op2 > 0
                    UNION ALL
                    SELECT op3, ibc_ej_ok, bur_ej_ok, totalt, skiftraknare FROM per_shift WHERE op3 IS NOT NULL AND op3 > 0
                )
                SELECT a.op_num,
                ...
                HAVING SUM(totalt) > 0
                ORDER BY (SUM(ibc_ej_ok + bur_ej_ok)) / SUM(totalt) DESC
                LIMIT 10
            ";
    ...
    private function getSkiftDuell(): void {
        try {
            ...
            $teamSql = "
                SELECT SUM(ibc_ok) AS tot_ibc, SUM(drifttid) AS tot_min
                FROM (
                    SELECT skiftraknare,
    ...
    private function getVeckoplanering(): void
    {
        try {
            ...
            $stmtWeek = $this->pdo->prepare("
                ...
                GROUP BY skiftraknare
                ORDER BY DATE(MAX(datum)), skiftraknare
            ");
