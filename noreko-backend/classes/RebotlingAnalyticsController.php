...
class RebotlingAnalyticsController {
    ...
    public function getProductionReport() {
        ...
        try {
            // ibc_ok/ibc_ej_ok/bur_ej_ok reset per skiftraknare — MAX() per skiftraknare is correct.
            // runtime_plc also resets per shift, so MAX() per shift is correct.
            $stmt = $this->pdo->prepare("
                SELECT
                    ...
                    ROUND(
                        CASE WHEN SUM(ibc_ok) + SUM(ibc_ej_ok) > 0
    ...
    public function getWeekComparison() {
        try {
            ...
            if ($granularity === 'shift') {
                ...
                // ibc_ok resets per skiftraknare — MAX() per skiftraknare is correct.
            ...
            // ibc_ok resets per skiftraknare — MAX() per skiftraknare is correct.
            $stmt = $this->pdo->prepare("
                SELECT dag, SUM(ibc_ok) AS ibc_ok, SUM(cykler) AS cykler
                FROM (
                    SELECT DATE(MAX(datum)) AS dag, skiftraknare,
                           COUNT(*)                  AS cykler,
                           MAX(COALESCE(ibc_ok, 0))  AS ibc_ok
    ...
    public function getExecDashboard() {
            ...
            } catch (\Throwable $e) { error_log('RebotlingAnalyticsController::getExecDashboard settings: ' . $e->getMessage()); }
            ...
            $oeeYesterday = 0;
            try {
                // ibc_ok/ibc_ej_ok are PLC daily running counters (don't reset per shift).
                ...
                $comboRow = $this->pdo->query("
                    ...
                    FROM (
                        SELECT ibc_t.ibc_ok, ibc_t.ibc_ej_ok, rt_t.runtime_min, rt_t.rast_min
                        FROM (
                            SELECT MAX(COALESCE(ibc_ok,0)) AS ibc_ok, MAX(COALESCE(ibc_ej_ok,0)) AS ibc_ej_ok
                            FROM rebotling_ibc
                            WHERE datum >= CURDATE() AND datum < CURDATE() + INTERVAL 1 DAY AND ibc_ok IS NOT NULL
                        ) ibc_t
                        CROSS JOIN (
                            SELECT SUM(shift_runtime) AS runtime_min, SUM(shift_rast) AS rast_min
                            FROM (
                                SELECT MAX(COALESCE(runtime_plc,0)) AS shift_runtime, MAX(COALESCE(rasttime,0)) AS shift_rast
                                FROM rebotling_ibc
                                WHERE datum >= CURDATE() AND datum < CURDATE() + INTERVAL 1 DAY AND ibc_ok IS NOT NULL
                                GROUP BY skiftraknare
                            ) ps_rt
                    ...
                    CROSS JOIN (
                        SELECT ibc_y.ibc_ok, ibc_y.ibc_ej_ok, rt_y.runtime_min, rt_y.rast_min
                        FROM (
                            SELECT MAX(COALESCE(ibc_ok,0)) AS ibc_ok, MAX(COALESCE(ibc_ej_ok,0)) AS ibc_ej_ok
                            FROM rebotling_ibc
                            WHERE datum >= DATE_SUB(CURDATE(), INTERVAL 1 DAY) AND datum < CURDATE() AND ibc_ok IS NOT NULL
                        ) ibc_y
                        CROSS JOIN (
                            SELECT SUM(shift_runtime) AS runtime_min, SUM(shift_rast) AS rast_min
                            FROM (
            ...
            } catch (\Throwable $e) { error_log('RebotlingAnalyticsController::execDashboard OEE combo: ' . $e->getMessage()); }
            ...
            try {
                ...
                if ($boRow) {
                    ...
                    } catch (\Throwable $e) { error_log('RebotlingAnalyticsController::getExecDashboard bestOp name: ' . $e->getMessage()); }
                    $bestOperator = [
                        'id'    => $opId,
            ...
            } catch (\Throwable $e) { error_log('RebotlingAnalyticsController::execDashboard bestOp: ' . $e->getMessage()); }
            ...
            } catch (\Throwable $e) { error_log('RebotlingAnalyticsController::execDashboard lastShiftOps: ' . $e->getMessage()); }
        ...
        try {
            ...
            $opStmt = $this->pdo->prepare("
                SELECT
                    ...
                    SUM(d.ibc_ok)    AS ibc_ok,
                    SUM(d.totalt)    AS totalt,
                    SUM(d.drifttid)  AS drifttid
                FROM (
                    SELECT skiftraknare,
    ...
    public function getSPC() {
        ...
        try {
            ...
            // ibc_ok is a daily running counter; MAX per skiftraknare + LAG() gives per-shift delta.
            // drifttid resets per shift so MAX per skiftraknare is correct without LAG.
            $stmt = $this->pdo->prepare("
            ...
            foreach ($rows as $r) {
                $ibcOk    = (int)$r['ibc_ok'];
                $driftMin = (float)$r['max_drift'];
                if ($driftMin > 0 && $ibcOk > 0) {
            ...
            if (empty($points)) {
                $stmt2 = $this->pdo->prepare("
                    ...
                    WHERE datum >= DATE_SUB(CURDATE(), INTERVAL :days2 DAY)
                      AND ibc_ok IS NOT NULL
                    GROUP BY DATE(datum), skiftraknare
                    HAVING shift_runtime > 0 AND ibc_ok > 0
                    ORDER BY dag ASC, skiftraknare ASC
                ");
                $stmt2->execute(['days2' => $days]);
                $rows2 = $stmt2->fetchAll(PDO::FETCH_ASSOC);
                foreach ($rows2 as $r) {
                    $ibcPerH = round($r['ibc_ok'] * 60.0 / $r['shift_runtime'], 2);
                    $points[] = [
    ...
    public function getYearCalendar() {
        ...
        try {
            ...
            } catch (\Throwable $e) { error_log('RebotlingAnalyticsController::getYearCalendar weekdayGoals: ' . $e->getMessage()); }
    ...
    public function getDayDetail() {
        ...
        try {
            ...
            // ibc_ok is a daily running counter — it does NOT reset at shift boundaries, only at midnight.
            // rawRows is ordered by (skiftraknare, timme), so we pre-seed each new shift's starting
            // accumulator with the previous shift's final value to get correct per-shift-hour deltas.
            $shiftPrevIbc   = [];
            $shiftPrevEjOk  = [];
            ...
            foreach ($rawRows as $r) {
                ...
                // Seed new shift's starting value from previous shift's final accumulator
                if ($sk !== $prevSk && !isset($shiftPrevIbc[$sk])) {
                    $shiftPrevIbc[$sk]  = ($prevSk !== null) ? $prevSkLastIbc  : 0;
                    $shiftPrevEjOk[$sk] = ($prevSk !== null) ? $prevSkLastEjOk : 0;
                }

                // Delta IBC
                $delta = max(0, $acc - $shiftPrevIbc[$sk]);
                $shiftPrevIbc[$sk] = $acc;

                // Delta ej_ok
                $deltaEo = max(0, $eo - $shiftPrevEjOk[$sk]);
                $shiftPrevEjOk[$sk] = $eo;
    ...
    public function getBenchmarking() {
        try {
            ...
            // OEE-beräkningsfunktion (lokal closure)
            $calcOee = function(float $ibcOk, float $ibcEjOk, float $runtimeMin, float $rastMin) use ($idealRatePerMin): ?float {
                $totalIBC = $ibcOk + $ibcEjOk;
                $runtimeM = max($runtimeMin, 1);
                $plannedM = max($runtimeMin + $rastMin, 1);
                $avail    = min($runtimeM / $plannedM, 1.0);
                $perf     = min(($totalIBC / $runtimeM) / $idealRatePerMin, 1.0);
                $qual     = $totalIBC > 0 ? $ibcOk / $totalIBC : 0;
                return round($avail * $perf * $qual * 100, 1);
            };

            // ================================================================
            // 1. Konsoliderad query: veckodata + OEE (alla veckor) med CTE
            //    Reducerar 2 fulla tabellscanningar till 1
            // ================================================================
            // ibc_ok in rebotling_ibc is a daily running counter (does not reset per shift).
            // LAG() over skiftraknare within each day gives the true per-shift delta.
            $stmtWeeks = $this->pdo->query("
                WITH per_shift AS (
                    SELECT
                        DATE(datum)                     AS dag,
                        YEAR(datum)                     AS yr,
                ...
                SELECT
                    yr, wk,
                    SUM(ibc_ok)                                                                      AS ibc_total,
                    ROUND(SUM(ibc_ok) * 100.0 / NULLIF(SUM(ibc_ok) + SUM(ibc_ej_ok), 0), 1)        AS avg_quality,
                    COUNT(DISTINCT dag)                                                               AS days_active,
                    SUM(ibc_ok)                                                                      AS ibc_ok,
                    SUM(ibc_ej_ok)                                                                   AS ibc_ej_ok,
                    SUM(shift_runtime)                                                               AS runtime_min,
                    SUM(shift_rast)                                                                  AS rast_min
            ...
            ");
            $allWeeksRaw = $stmtWeeks->fetchAll(PDO::FETCH_ASSOC);

            // ---- Bygg topp-10-lista + OEE + hitta innevarande vecka ----
            $curYr = (int)$now->format('Y');
            $curWk = (int)$now->format('W');
            $topWeeks = [];
            $currentWeek = null;

            foreach ($allWeeksRaw as $row) {
                $yr  = (int)$row['yr'];
                $wk  = (int)$row['wk'];
                $oee = ((float)$row['runtime_min'] > 0)
                    ? $calcOee((float)$row['ibc_ok'], (float)$row['ibc_ej_ok'], (float)$row['runtime_min'], (float)$row['rast_min'])
                    : null;
                $weekLabel = 'V' . $wk . ' ' . $yr;
                $entry = [
                ...
                if (count($topWeeks) < 10) {
                    $topWeeks[] = $entry;
                }

                if ($yr === $curYr && $wk === $curWk) {
                    $ibcTotal   = (int)$row['ibc_total'];
                    $daysActive = (int)$row['days_active'];
                    $currentWeek = [
                        'week_label'  => $weekLabel,
                        'ibc_total'   => $ibcTotal,
                        'ibc_per_day' => $daysActive > 0 ? round($ibcTotal / $daysActive, 1) : 0.0,
                        'avg_quality' => (float)$row['avg_quality'],
                        'avg_oee'     => $oee,
            ...
            // Fallback om innevarande vecka inte har data
            if ($currentWeek === null) {
                $currentWeek = [
                    'week_label'  => 'V' . $curWk . ' ' . $curYr,
                    'ibc_total'   => 0,
                    'ibc_per_day' => 0.0,
                    'avg_quality' => 0.0,
                    'avg_oee'     => null,
            ...
            // ================================================================
            // 3. Bästa dagen + månadsöversikt (konsoliderad CTE, 1 query)
            // ================================================================
            $stmtDayMonth = $this->pdo->query("
                WITH per_shift AS (
                    SELECT
                        DATE(datum)                     AS dag,
                        DATE_FORMAT(datum, '%Y-%m')     AS ym,
                        skiftraknare,
                        COALESCE(MAX(ibc_ok), 0)        AS ibc_ok,
                        COALESCE(MAX(ibc_ej_ok), 0)     AS ibc_ej_ok
                    FROM rebotling_ibc
                ...
                per_day AS (
                    SELECT
                        dag, ym,
                        SUM(ibc_ok)    AS ibc_total,
                        ROUND(SUM(ibc_ok) * 100.0 / NULLIF(SUM(ibc_ok) + SUM(ibc_ej_ok), 0), 1) AS quality
                    FROM per_shift
                    GROUP BY dag
                )
                SELECT dag, ym, ibc_total, quality
                FROM per_day
                ORDER BY ibc_total DESC
            ");
            $allDays = $stmtDayMonth->fetchAll(PDO::FETCH_ASSOC);
            ...
            if (!empty($allDays)) {
                $bestDayEver = [
                    'date'      => $allDays[0]['dag'],
                    'ibc_total' => (int)$allDays[0]['ibc_total'],
                    'quality'   => (float)$allDays[0]['quality'],
                ];
            }
    ...
    public function getMonthCompare() {
        try {
            ...
            // Hjälpfunktion: hämta summering för en månad (1 query med CTE istf 2)
            ...
            try {
                ...
                $rankSQL = "
                    ...
                    ORDER BY (
                        SUM(ibc_ok) * 0.6
                        + SUM(ibc_ok) / NULLIF(SUM(runtime_h), 0) * 100 * 0.25
                        + LEAST(100.0, SUM(ibc_ok)*100.0 / NULLIF(SUM(ibc_ok + ibc_ej_ok), 0)) * 0.15
                    ) DESC
                    LIMIT 10
                ";
    ...
    public function getMonthlyReport() {
        try {
            ...
            // Använder rebotling_ibc (samma källa som månadssammanfattningen) för
            // konsistenta IBC-värden. DEDUP: samma operator op1+op2+op3 räknas en
            // gång per skift via GROUP BY (op_num, skiftraknare).
            $opSQL = "
                WITH per_shift AS (
                    SELECT skiftraknare,
                           MAX(COALESCE(op1, 0))          AS op1,
                           ...
                           MAX(COALESCE(ibc_ok,    0))    AS ibc_ok,
                           MAX(COALESCE(ibc_ej_ok, 0))    AS ibc_ej_ok,
                           MAX(COALESCE(runtime_plc, 0))  AS drifttid
                    FROM rebotling_ibc
                    WHERE datum >= :first_day AND datum < :next_month
                      AND ibc_ok IS NOT NULL
                    GROUP BY skiftraknare
                ),
                all_ops AS (
                    SELECT skiftraknare, op1 AS op_num, ibc_ok, ibc_ej_ok, drifttid FROM per_shift WHERE op1 > 0
                    UNION ALL
                    SELECT skiftraknare, op2, ibc_ok, ibc_ej_ok, drifttid FROM per_shift WHERE op2 > 0
                    UNION ALL
                    SELECT skiftraknare, op3, ibc_ok, ibc_ej_ok, drifttid FROM per_shift WHERE op3 > 0
                ),
                deduped AS (
                ...
                SELECT
                    ...
                    SUM(t.ibc_ok + t.ibc_ej_ok)    AS totalt,
                    SUM(t.drifttid)                AS drifttid,
                    COUNT(DISTINCT t.skiftraknare) AS shifts
                FROM deduped t
                JOIN operators o ON o.number = t.op_num
                GROUP BY o.number, o.name
                HAVING SUM(t.ibc_ok) > 0
                ORDER BY (
                    SUM(t.ibc_ok) * 0.6
                    + (SUM(t.ibc_ok) / NULLIF(SUM(t.drifttid)/60.0, 0)) * 100 * 0.25
                    + LEAST(100.0, SUM(t.ibc_ok)*100.0 / NULLIF(SUM(t.ibc_ok + t.ibc_ej_ok), 0)) * 0.15
                ) DESC
                LIMIT 20
            ";
            $stmt = $this->pdo->prepare($opSQL);
            $stmt->execute(['first_day' => $firstDay, 'next_month' => $nextMonth]);
    ...
    public function getQualityTrend() {
        ...
        try {
            $stmt = $this->pdo->prepare("
                ...
                SELECT dag,
                       ROUND(SUM(ibc_ok) * 100.0 / NULLIF(SUM(ibc_ok) + SUM(ibc_ej_ok), 0), 1) AS quality_pct,
                       SUM(ibc_ok) AS ibc_ok,
                       SUM(ibc_ok) + SUM(ibc_ej_ok) AS ibc_totalt
                FROM per_shift
                GROUP BY dag
                ORDER BY dag ASC
            ");
    ...
    public function getWeekdayStats() {
        ...
        try {
            // ibc_ok/ibc_ej_ok are cumulative PLC counters — use LAG() delta per shift.
            // runtime_plc/rasttime reset per shift so MAX() per shift is correct.
            $stmt = $this->pdo->prepare("
                WITH per_shift AS (
                    SELECT DATE(datum) AS dag, skiftraknare,
                           MAX(COALESCE(ibc_ok,    0)) AS ibc_ok,
    ...
    public function getSkiftrapportList() {
        try {
            ...
            // KPI-sammanfattning — ibc_ok resets per skiftraknare, MAX() per skiftraknare is correct.
            $kpiSql = "
                WITH lag_shifts AS (
                    SELECT s.skiftraknare,
                           MAX(s.op1)      AS op1,
                           MAX(s.op2)      AS op2,
                           MAX(s.op3)      AS op3,
                           MAX(s.totalt)   AS totalt,
                           MAX(s.drifttid) AS drifttid,
                           MAX(s.ibc_ok)   AS ibc_ok
                    FROM rebotling_skiftrapport s
                    GROUP BY s.skiftraknare
                )
                SELECT
                    COALESCE(SUM(ls.ibc_ok), 0)     AS total_ibc_ok,
...
HTML;
        ...
        try {
            ...
            $stmtDaily = $this->pdo->prepare("
                ...
                SELECT dag,
                       ...
                       SUM(ibc_ok) + SUM(ibc_ej_ok) AS ibc_totalt,
                       SUM(ibc_ej_ok) AS ibc_ej_ok,
                       ROUND(SUM(ibc_ok) * 100.0 / NULLIF(SUM(ibc_ok) + SUM(ibc_ej_ok), 0), 1) AS kvalitet_pct
                FROM per_shift
                GROUP BY dag
                ORDER BY dag ASC
            ");
    ...
    public function getShiftPdfSummary() {
        ...
        try {
            ...
            } catch (\Throwable $e) { error_log('ShiftPdf products: ' . $e->getMessage()); }
            ...
            try {
                ...
                $komRow = $komStmt->fetch(PDO::FETCH_ASSOC);
                if ($komRow) $kommentar = $komRow['kommentar'] ?? '';
            } catch (\Throwable $e) { error_log("RebotlingAnalyticsController::getShiftPdfSummary kommentar: " . $e->getMessage()); }
            ...
            try {
                ...
                $lopStmt = $this->pdo->prepare("
                    SELECT DISTINCT lopnummer FROM rebotling_ibc
                    WHERE skiftraknare IN ($phLop)
                      AND lopnummer IS NOT NULL AND lopnummer > 0 AND lopnummer < 900
                ...
                if (!empty($lopRows)) {
                    $ranges = [];
                    $start = (int)$lopRows[0]; $prev = $start;
            ...
            } catch (\Throwable $e) { error_log('ShiftPdfSummary löpnummer: ' . $e->getMessage()); }
            ...
            } catch (\Throwable $e) { error_log('ShiftPdfSummary graf/stopp: ' . $e->getMessage()); }
    ...
    private function computeWeeklySummary(string $week): array {
        ...
        $stmtOps = $this->pdo->prepare("
            ...
            FROM (
                SELECT DATE(datum) AS dag, skiftraknare,
                       MAX(COALESCE(op1,        0)) AS operator_id,
                       MAX(COALESCE(ibc_ok,     0)) AS ibc_ok,
                       MAX(COALESCE(ibc_ej_ok,  0)) AS ibc_ej_ok,
                       MAX(COALESCE(runtime_plc, 0)) AS shift_runtime
                FROM rebotling_ibc
    ...
    public function getProductionGoalProgress(): void {
        try {
            ...
            if ($period === 'week') {
                ...
                $stmt = $this->pdo->prepare(
                    "SELECT COALESCE(SUM(ibc_ok), 0) AS cnt FROM (SELECT DATE(datum) AS dag, skiftraknare, MAX(COALESCE(ibc_ok, 0)) AS ibc_ok FROM rebotling_ibc WHERE datum BETWEEN ? AND ? AND produktion_procent > 0 GROUP BY DATE(datum), skiftraknare) base"
                );
