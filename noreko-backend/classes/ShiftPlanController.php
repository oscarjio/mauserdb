...
class ShiftPlanController {
    ...
    private function assign() {
        ...
        $note     = isset($data['note'])       ? mb_substr(strip_tags(trim($data['note'])), 0, 500)  : null;
    ...
    private function copyWeek() {
        ...
        try {
            ...
            if (empty($rows)) {
                http_response_code(404);
                echo json_encode(['success' => false, 'error' => 'Inga tilldelningar att kopiera från föregående vecka'], JSON_UNESCAPED_UNICODE);
                return;
            }
