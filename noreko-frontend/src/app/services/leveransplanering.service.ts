...
export class LeveransplaneringService {
  ...
  skapaOrder(order: any): Observable<SkapaOrderResponse | null> {
    return this.http.post<SkapaOrderResponse>(
      ...
      order,
      { withCredentials: true }
    ).pipe(timeout(15000), catchError(err => { console.error('skapaOrder failed', err); return of({ success: false, data: null as any, timestamp: '', error: err?.error?.error || 'Nätverksfel' }); }));
  }
  ...
  uppdateraOrder(id: number, status: string): Observable<UppdateraOrderResponse | null> {
    return this.http.post<UppdateraOrderResponse>(
      ...
      { id, status },
      { withCredentials: true }
    ).pipe(timeout(15000), catchError(err => { console.error('uppdateraOrder failed', err); return of({ success: false, data: null as any, timestamp: '', error: err?.error?.error || 'Nätverksfel' }); }));
  }
}
