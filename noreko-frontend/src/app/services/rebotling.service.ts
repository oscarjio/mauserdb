...
export class RebotlingService {
  ...
  getQualityTrend(days: number = 30): Observable<any> {
  ...
  getStoppageAnalysis(days: number = 30): Observable<any> {
  ...
  getWeeklySummary(week: string): Observable<any> {
  ...
  getShiftDayNightComparison(days: number = 30): Observable<any> {
  ...
  // ---- Bonus-simulator ----
  getBonusSimulator(days: number = 30, params?: BonusSimulatorParams): Observable<any> {
  ...
  getTopOperatorsLeaderboard(days: number = 30): Observable<any> {
  ...
  getMinDagSummary(operatorId?: number): Observable<any> {
  ...
  getKassationsOverview(days: number): Observable<any> {
  ...
  getOperatorsForCompare(): Observable<any> {
  ...
  getHourlyHeatmap(period: number): Observable<any> {
...
// ---- Kassationsanalys utokade interfaces ----

export interface KassationsOverviewData {
  days: number;
  period_from: string;
  period_to: string;
  total_kasserade: number;
  total_registrerade: number;
  prev_kasserade: number;
  antal_trend: 'up' | 'down' | 'stable';
  antal_diff: number;
  kassationsgrad_pct: number;
  prev_grad_pct: number;
  grad_trend: 'up' | 'down' | 'stable';
  grad_diff: number;
  vanligaste_orsak: string | null;
  vanligaste_antal: number;
  uppskattad_kostnad: number;
  kostnad_per_ibc: number;
  total_produktion: number;
  prev_produktion: number;
}

export interface KassationsByPeriodData {
  days: number;
  group: string;
  from: string;
  to: string;
...
export interface KassationsDetailRow {
  id: number;
  datum: string;
  skiftraknare: number | null;
  antal: number;
...
export interface KassationsDetailsData {
  days: number;
  from: string;
  to: string;
  total: number;
  detaljer: KassationsDetailRow[];
  orsaker: { id: number; namn: string }[];
  operatorer: string[];
  har_data: boolean;
}

export interface KassationsTrendRateData {
  days: number;
  from: string;
  to: string;
  labels: string[];
