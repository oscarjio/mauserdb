...
export class RebotlingSkiftrapportPage implements OnInit, OnDestroy {
  ...
  getEfficiency(r: any): number | null {
    ...
    const targetCycleMin = product?.cycle_time_minutes || 3;
    const targetIbcH = 60 / targetCycleMin;
    return Math.min(Math.round((ibcH / targetIbcH) * 100), 100);
  }
  ...
  fetchProducts() {
    this.skiftrapportService.getProducts().pipe(timeout(15000), catchError(err => { console.error('Fel vid hämtning av produkter:', err); return of(null); }), takeUntil(this.destroy$)).subscribe({
      next: (res) => {
        if (res?.success) {
  ...
  fetchReports(silent: boolean = false) {
    ...
    this.fetchSub = this.skiftrapportService.getSkiftrapporter()
      .pipe(timeout(15000), catchError(err => { console.error('Fel vid hämtning av skiftrapporter:', err); return of({ success: false, message: 'Kunde inte hämta skiftrapporter', data: [] }); }), takeUntil(this.destroy$))
      .subscribe({
        next: (res) => {
          if (!silent) {
            this.loading = false;
  ...
  toggleInlagd(report: any) {
    ...
    this.skiftrapportService.updateInlagd(report.id, newInlagd).pipe(timeout(15000), catchError(err => { console.error('Fel vid uppdatering av inlagd-status:', err); return of({ success: false, error: 'Ett fel uppstod' }); }), takeUntil(this.destroy$)).subscribe({
      next: (res) => {
        if (res.success) {
  ...
  bulkMarkInlagd(inlagd: boolean) {
    ...
    this.skiftrapportService.bulkUpdateInlagd(ids, inlagd).pipe(timeout(15000), catchError(err => { console.error('Fel vid massuppdatering av inlagd-status:', err); return of({ success: false, error: 'Ett fel uppstod' }); }), takeUntil(this.destroy$)).subscribe({
      next: (res) => {
        if (res.success) {
  ...
  deleteReport(id: number) {
    ...
    this.skiftrapportService.deleteSkiftrapport(id).pipe(timeout(15000), catchError(err => { console.error('Fel vid borttagning av skiftrapport:', err); return of({ success: false, error: 'Ett fel uppstod' }); }), takeUntil(this.destroy$)).subscribe({
      next: (res) => {
        if (res.success) {
  ...
  bulkDelete() {
    ...
    const ids = Array.from(this.selectedIds);
    this.skiftrapportService.bulkDelete(ids).pipe(timeout(15000), catchError(err => { console.error('Fel vid massborttagning:', err); return of({ success: false, error: 'Ett fel uppstod' }); }), takeUntil(this.destroy$)).subscribe({
      next: (res) => {
        if (res.success) {
  ...
  addReport() {
    ...
    this.skiftrapportService.createSkiftrapport({
      ...
      totalt:      totalt
    }).pipe(timeout(15000), catchError(err => { console.error('Fel vid skapande av skiftrapport:', err); this.loading = false; this.addingReport = false; return of({ success: false, error: 'Ett fel uppstod vid skapande av skiftrapport' }); }), takeUntil(this.destroy$)).subscribe({
      next: (res) => {
        this.loading = false;
  ...
  private loadLopnummer(report: any) {
    ...
    this.skiftrapportService.getLopnummer(report.skiftraknare, report.datum || report.start_datum)
      .pipe(timeout(15000), catchError(err => { console.error('Fel vid laddning av löpnummer:', err); return of(null); }), takeUntil(this.destroy$))
      .subscribe({
        next: (res) => {
  ...
  saveReport(report: any) {
    ...
    }).pipe(timeout(15000), catchError(err => { console.error('Fel vid uppdatering av skiftrapport:', err); return of({ success: false, error: 'Ett fel uppstod vid uppdatering' }); }), takeUntil(this.destroy$)).subscribe({
      next: (res) => {
        if (res.success) {
  ...
  laddaKommentar(report: any) {
    ...
    .pipe(timeout(15000), catchError(err => { console.error('Fel vid laddning av kommentar:', err); return of(null); }), takeUntil(this.destroy$))
    .subscribe({
      next: (res) => {
  ...
  sparaKommentar(report: any) {
    ...
    .pipe(timeout(15000), catchError(err => { console.error('Fel vid sparande av kommentar:', err); this.spararKommentar[id] = false; this.errorMessage = 'Serverfel vid sparande av kommentar'; return of(null); }), takeUntil(this.destroy$))
    .subscribe({
      next: (res) => {
        if (!res) return;
  ...
  compareShifts() {
    ...
    .pipe(timeout(15000), catchError(err => { console.error('Fel vid skiftjämförelse:', err); this.compareLoading = false; this.compareError = 'Serverfel vid jämförelse'; return of(null); }), takeUntil(this.destroy$))
    .subscribe({
      next: (res) => {
        if (!res) return;
