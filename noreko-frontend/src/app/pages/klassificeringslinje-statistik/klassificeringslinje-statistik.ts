...
export class KlassificeringslinjeStatistikPage implements OnInit, AfterViewInit, OnDestroy {
  ...
  loadOeeTrend() {
    ...
    const url = `${this.apiBase}?action=klassificeringslinje&run=oee-trend&dagar=${this.oeeTrendDagar}`;
  ...
  exportCSV() {
    ...
    link.href = url;
    link.download = `klassificeringslinje-statistik-${new Date().toISOString().split('T')[0]}.csv`;
    link.click();
  ...
  exportExcel() {
    ...
    import('xlsx').then(XLSX => {
      ...
      XLSX.writeFile(wb, `klassificeringslinje-statistik-${new Date().toISOString().split('T')[0]}.xlsx`);
    });
  }
}
