...
@Component({
  standalone: true, selector: 'app-rebotling-quality-trend',
  templateUrl: './rebotling-quality-trend.component.html',
  imports: [CommonModule, FormsModule]
})
export class RebotlingQualityTrendComponent implements OnInit, OnDestroy {
  qualityTrendDays = 30;
  qualityTrendLoaded = false;
  qualityTrendLoading = false;
  ...
  loadQualityTrend() {
    ...
    this.rebotlingService.getQualityTrend(this.qualityTrendDays).pipe(
      ...
      } else { this.qualityTrendLoaded = true; }
    });
  }
  ...
  private renderQualityTrendChart() {
    ...
    this.qualityTrendChart = new Chart(canvas, {
      ...
      data: { labels, datasets: [
        { label: 'Daglig kvalitet %', data: dailyData, borderColor: 'rgba(236,201,75,0.9)', backgroundColor: 'rgba(236,201,75,0.12)',
          tension: 0.3, pointRadius: 3, borderWidth: 2, fill: true, spanGaps: true, yAxisID: 'y' },
        { label: '7-dagars rullande snitt', data: rollingData, borderColor: 'rgba(237,137,54,1)', backgroundColor: 'transparent',
          tension: 0.4, pointRadius: 1, borderWidth: 3, fill: false, spanGaps: true, yAxisID: 'y' },
        { label: 'Kvalitetsmål 90%', data: targetLine, borderColor: 'rgba(252,129,129,0.8)', backgroundColor: 'transparent',
      ...
      options: {
        ...
        scales: {
          ...
          y: { min: 0, max: 100, ticks: { color: '#718096', callback: (v: any) => v + '%' }, grid: { color: 'rgba(255,255,255,0.06)' },
