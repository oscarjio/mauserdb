...
@Component({
  standalone: true, selector: 'app-rebotling-events-admin',
  templateUrl: './rebotling-events-admin.component.html',
  imports: [CommonModule, FormsModule]
})
export class RebotlingEventsAdminComponent implements OnInit, OnDestroy {
  @Output() eventsChanged = new EventEmitter<ProductionEvent[]>();

  isAdmin = false;
  productionEvents: ProductionEvent[] = [];
  productionEventsLoading = false;
  showEventsAdmin = false;
  newEvent = { event_date: '', title: '', event_type: 'ovrigt', description: '' };
  eventsAdminMessage = '';
  eventsAdminError = '';
  eventsAdminSaving = false;
  ...
  ngOnInit() {
    ...
    if (this.isAdmin) { this.loadProductionEvents(); }
  }
  ...
  eventTypeLabel(type: string): string {
    const labels: Record<string, string> = {
      'underhall': 'Underh\u00e5ll', 'ny_operator': 'Ny operat\u00f6r', 'mal_andring': 'M\u00e5l\u00e4ndring',
      'rekord': 'Rekord', 'ovrigt': '\u00d6vrigt'
    };
  ...
  loadProductionEvents(): void {
    ...
    this.rebotlingService.getProductionEvents(fmt(startDate), fmt(endDate)).pipe(
      ...
      if (res?.success && res.events) {
        ...
        this.eventsChanged.emit(this.productionEvents);
      }
    });
  }
    ...
    this.rebotlingService.addProductionEvent(this.newEvent).pipe(
      ...
      if (res?.success) {
        const added: ProductionEvent = {
          id: res.id, event_date: this.newEvent.event_date,
          title: this.newEvent.title, description: this.newEvent.description,
