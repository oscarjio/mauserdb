...
@Component({
  standalone: true,
  selector: 'app-rebotling-histogram',
  templateUrl: './rebotling-histogram.component.html',
  imports: [CommonModule, FormsModule]
})
export class RebotlingHistogramComponent implements OnInit, OnDestroy {
  ...
  ngOnInit() { this.loadCycleHistogram(); }
  ...
  private renderHistogramChart() {
    ...
    this.histogramChart = new Chart(canvas, {
      ...
      data: {
        labels,
        datasets: [{
          label: 'Antal skift', data: counts,
          backgroundColor: 'rgba(72, 187, 120, 0.75)', borderColor: '#48bb78',
      ...
      options: {
        ...
        plugins: {
          ...
          tooltip: {
            backgroundColor: 'rgba(20,20,30,0.95)', titleColor: '#fff',
            bodyColor: '#e2e8f0', borderColor: '#48bb78', borderWidth: 1,
            callbacks: { label: (ctx: any) => ` ${ctx.parsed.y} st` }
          }
        },
        scales: {
          x: { ticks: { color: '#a0aec0' }, grid: { color: 'rgba(255,255,255,0.05)' } },
          y: { beginAtZero: true, ticks: { color: '#a0aec0', stepSize: 1 }, grid: { color: 'rgba(255,255,255,0.05)' },
