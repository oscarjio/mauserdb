...
@Component({
  standalone: true, selector: 'app-rebotling-weekday-stats',
  templateUrl: './rebotling-weekday-stats.component.html',
  imports: [CommonModule, FormsModule]
})
export class RebotlingWeekdayStatsComponent implements OnInit, OnDestroy {
  ...
  getWeekdayMaxIbc(): number { return this.weekdayData.length ? Math.max(...this.weekdayData.map(d => d.snitt_ibc)) : 0; }
  getWeekdayMinIbc(): number { return this.weekdayData.length ? Math.min(...this.weekdayData.map(d => d.snitt_ibc)) : 0; }

  loadWeekdayStats(): void {
    this.weekdayLoading = true;
    this.rebotlingService.getWeekdayStats(this.weekdayDagar).pipe(
      timeout(8000), catchError(() => of({ success: false, veckodagar: [] })), takeUntil(this.destroy$)
    ).subscribe(r => {
  ...
  private buildWeekdayChart(): void {
    ...
    const colors = ibcData.map(v => v === maxIbc ? 'rgba(72, 187, 120, 0.85)' : v === minIbc ? 'rgba(245, 101, 101, 0.85)' : 'rgba(66, 153, 225, 0.65)');
    this.weekdayChart = new Chart(ctx, {
      type: 'bar',
      data: { labels, datasets: [{ label: 'Snitt IBC/dag', data: ibcData, backgroundColor: colors,
        borderColor: colors.map(c => c.replace('0.85', '1').replace('0.65', '1')), borderWidth: 1, borderRadius: 4 }] },
      options: {
        responsive: true,
        ...
        scales: {
          ...
          y: { beginAtZero: true, ticks: { color: '#a0aec0' }, grid: { color: '#4a5568' }, title: { display: true, text: 'IBC/dag', color: '#a0aec0' } }
        }
      }
    });
  }
