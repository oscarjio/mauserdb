...
@Component({
  standalone: true, selector: 'app-rebotling-oee-waterfall',
  templateUrl: './rebotling-oee-waterfall.component.html',
  imports: [CommonModule, FormsModule]
})
export class RebotlingOeeWaterfallComponent implements OnInit, OnDestroy {
  oeeWaterfallDays = 30;
  oeeWaterfallLoaded = false;
  oeeWaterfallLoading = false;
  ...
  private renderOeeWaterfallChart() {
    ...
    this.oeeWaterfallChart = new Chart(canvas, {
      ...
      data: { labels, datasets: [
        { label: 'Uppnått', data: achieved, backgroundColor: achieved.map(v => v >= 85 ? 'rgba(72,187,120,0.85)' : v >= 65 ? 'rgba(236,201,75,0.85)' : 'rgba(252,129,129,0.85)'),
        ...
        { label: 'Förlust', data: losses, backgroundColor: 'rgba(255,255,255,0.06)', borderColor: 'rgba(255,255,255,0.12)',
          borderWidth: 1, borderRadius: 4, stack: 'oee' }
      ] },
      options: {
        ...
        plugins: {
          ...
          tooltip: { backgroundColor: 'rgba(15,17,23,0.95)', titleColor: '#fff', bodyColor: '#e0e0e0', borderColor: '#48bb78', borderWidth: 1,
            callbacks: { label: (ctx: any) => { const v = ctx.parsed.x; return ctx.datasetIndex === 0 ? `Uppnått: ${v.toFixed(1)}%` : `Förlust: ${v.toFixed(1)}%`; } } }
        },
        scales: {
