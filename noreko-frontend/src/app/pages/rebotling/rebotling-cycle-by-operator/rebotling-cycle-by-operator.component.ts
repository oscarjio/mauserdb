...
@Component({
  standalone: true,
  selector: 'app-rebotling-cycle-by-operator',
  templateUrl: './rebotling-cycle-by-operator.component.html',
  imports: [CommonModule, FormsModule]
})
export class RebotlingCycleByOperatorComponent implements OnInit, OnDestroy {
  cycleByOpDays = 30;
  cycleByOpLoaded = false;
  cycleByOpLoading = false;
  ...
  ngOnInit() { this.loadCycleByOperator(); }
  ngOnDestroy() {
  ...
  onCycleByOpDaysChange() { this.cycleByOpLoaded = false; this.loadCycleByOperator(); }
  ...
  private renderCycleByOpChart() {
    ...
    const colors = chartData.map(op => {
      ...
      return v <= teamSnitt ? 'rgba(72, 187, 120, 0.8)' : 'rgba(252, 129, 129, 0.8)';
    });
    this.cycleByOpChart = new Chart(canvas, {
      type: 'bar',
      data: { labels, datasets: [{ label: 'Median cykeltid (min)', data: values, backgroundColor: colors,
        borderColor: colors.map(c => c.replace('0.8)', '1)')), borderWidth: 1, borderRadius: 4 }] },
      options: {
        indexAxis: 'y', responsive: true, maintainAspectRatio: false,
        plugins: { legend: { display: false },
          tooltip: { backgroundColor: 'rgba(15,17,23,0.96)', titleColor: '#fff', bodyColor: '#e0e0e0',
            borderColor: '#4299e1', borderWidth: 1,
            callbacks: {
              title: (items: any[]) => chartData[items[0].dataIndex].namn,
              label: (ctx: any) => {
                const op = chartData[ctx.dataIndex];
                const vsSign = (op.vs_team_snitt ?? 0) >= 0 ? '+' : '';
                return [` Median: ${op.median_min?.toFixed(2) ?? '–'} min`, ` P90: ${op.p90_min?.toFixed(2) ?? '–'} min`,
                  ` vs Teamsnitt: ${vsSign}${op.vs_team_snitt ?? 0}%`, ` Antal skift: ${op.antal_skift} st`, ` Total IBC: ${op.total_ibc}`];
              }
            }
          }
        },
        scales: {
          x: { beginAtZero: true, max: xMax, ticks: { color: '#a0aec0', callback: (v: any) => v.toFixed(1) + ' min' },
            grid: { color: 'rgba(255,255,255,0.05)' }, title: { display: true, text: 'Median cykeltid (minuter)', color: '#a0aec0', font: { size: 12 } } },
