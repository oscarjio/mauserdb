...
export class BatchSparningPage implements OnInit, OnDestroy {
  ...
  loadActiveBatches(): void {
    ...
    this.svc.getActiveBatches().pipe(catchError(() => of(null)), takeUntil(this.destroy$)).subscribe(res => {
  ...
  completeBatch(batchId: number): void {
    ...
    this.svc.completeBatch(batchId).pipe(catchError(() => of(null)), takeUntil(this.destroy$)).subscribe(res => {
      if (res?.success) {
        ...
        this.completeError = res?.error || 'Kunde inte markera batchen som klar. Försök igen.';
      }
    });
  }
