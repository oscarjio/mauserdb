...
@Component({
  standalone: true,
  selector: 'app-rebotling-cycle-trend',
  templateUrl: './rebotling-cycle-trend.component.html',
  imports: [CommonModule]
})
export class RebotlingCycleTrendComponent implements OnInit, OnDestroy {
  @Input() productionEvents: ProductionEvent[] = [];

  cycleTrendLoaded = false;
  cycleTrendLoading = false;
  cycleTrendDays = 30;
  cycleTrendData: any[] = [];
  ...
  eventColor(type: string): string {
    const colors: Record<string, string> = {
      'underhall': '#f97316', 'ny_operator': '#3b82f6', 'mal_andring': '#a855f7',
      'rekord': '#eab308', 'ovrigt': '#6b7280',
    };
  ...
  loadCycleTrend() {
    ...
    if (this.chartAnnotations.length === 0) {
      ...
      this.rebotlingService.getAnnotations(fmt(startDate), fmt(endDate)).pipe(
        ...
        if (res?.success && res.annotations) {
          this.chartAnnotations = res.annotations.map(ann => ({
            date: ann.date, dateShort: ann.date.substring(5),
            type: ann.type as 'stopp' | 'low_production' | 'audit', label: ann.label
          }));
  ...
  private renderCycleTrendChart() {
    ...
    const cycleProductionEventAnnotations: ChartAnnotation[] = (this.productionEvents || [])
      .filter(e => labels.some(l => l.includes(e.event_date.substring(5))))
      .map(e => ({
        date: e.event_date, dateShort: e.event_date.substring(5),
        type: 'audit' as const, label: e.title, color: this.eventColor(e.event_type)
      } as any));
    ...
    this.cycleTrendChart = new Chart(canvas, {
      ...
      data: {
        ...
        datasets: [
          {
            label: 'IBC OK', data: ibcData,
            backgroundColor: 'rgba(66,153,225,0.6)', borderColor: 'rgba(99,179,237,1)',
            borderWidth: 1, borderRadius: 3, yAxisID: 'y'
          },
          {
            type: 'line' as any, label: 'IBC/h', data: ibcPerHour,
            borderColor: '#48bb78', backgroundColor: 'rgba(72,187,120,0.1)',
            tension: 0.3, pointRadius: 2, borderWidth: 2, fill: false, yAxisID: 'y2'
          }
        ]
      },
      options: {
        ...
        plugins: {
          ...
          tooltip: {
            backgroundColor: 'rgba(15,17,23,0.95)', titleColor: '#fff',
            bodyColor: '#e0e0e0', borderColor: '#4299e1', borderWidth: 1
          },
        ...
        scales: {
          ...
          y: { beginAtZero: true, position: 'left', ticks: { color: '#718096' }, grid: { color: 'rgba(255,255,255,0.04)' },
               title: { display: true, text: 'IBC OK', color: '#a0aec0', font: { size: 11 } } },
          y2: { beginAtZero: true, position: 'right', ticks: { color: '#48bb78', callback: (v: any) => v + '/h' },
                grid: { drawOnChartArea: false }, title: { display: true, text: 'IBC/h', color: '#48bb78', font: { size: 11 } } }
        }
      }
    });
  }
