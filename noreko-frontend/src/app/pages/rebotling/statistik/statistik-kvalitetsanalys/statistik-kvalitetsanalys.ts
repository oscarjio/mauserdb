...
@Component({
  standalone: true,
  selector: 'app-statistik-kvalitetsanalys',
  templateUrl: './statistik-kvalitetsanalys.html',
  imports: [CommonModule, FormsModule]
})
export class StatistikKvalitetsanalysComponent implements OnInit, OnDestroy {
  rejectionAnalysisDays: number = 30;
  rejectionAnalysisLoading: boolean = false;
  rejectionAnalysisLoaded: boolean = false;
  rejectionKpi: { kvalitet_idag: number | null; kvalitet_vecka: number | null; kasserade_idag: number; trend_vs_forra_veckan: 'up' | 'down' | 'stable'; trend_diff: number | null; } = { kvalitet_idag: null, kvalitet_vecka: null, kasserade_idag: 0, trend_vs_forra_veckan: 'stable', trend_diff: null };
  ...
  get rejectionTrendLabel(): string { const t = this.rejectionKpi.trend_vs_forra_veckan; const diff = this.rejectionKpi.trend_diff; const diffStr = diff !== null ? (diff > 0 ? '+'+diff : ''+diff)+' pp' : ''; if (t === 'up') return this.rejectionTrendArrow+' Stigande '+diffStr; if (t === 'down') return this.rejectionTrendArrow+' Fallande '+diffStr; return this.rejectionTrendArrow+' Stabil'; }
  ...
  loadRejectionAnalysis(): void {
    ...
    this.rebotlingService.getRejectionAnalysis(this.rejectionAnalysisDays).pipe(timeout(8000), takeUntil(this.destroy$), catchError(() => of(null)))
  ...
  exportRejectionCSV(): void {
    ...
    const rows = this.rejectionTrendData.map(d => [d.datum, d.kvalitet_pct !== null ? d.kvalitet_pct.toFixed(1) : '', d.glidande_snitt !== null ? d.glidande_snitt.toFixed(1) : '', d.ibc_ok, d.ibc_kasserade, d.ibc_totalt]);
