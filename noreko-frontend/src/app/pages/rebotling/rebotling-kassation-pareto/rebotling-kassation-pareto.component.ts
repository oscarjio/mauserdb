...
@Component({
  standalone: true, selector: 'app-rebotling-kassation-pareto',
  templateUrl: './rebotling-kassation-pareto.component.html',
  imports: [CommonModule]
})
export class RebotlingKassationParetoComponent implements OnInit, OnDestroy {
  kassationDays = 30;
  kassationLoading = false;
  kassationPareto: KassationItem[] = [];
  kassationTotalAntal = 0;
  kassationTotalProduktion = 0;
  kassationPct = 0;
  ...
  exportKassationCSV(): void {
    if (!this.kassationPareto.length) return;
    const headers = ['Orsak', 'Antal kassationer', 'Andel %', 'Kumulativ %'];
    const rows = this.kassationPareto.map(item => [
      item.namn, item.antal, item.pct.toFixed(1) + '%', item.kumulativ_pct.toFixed(1) + '%'
    ]);
    rows.push(['TOTALT', this.kassationTotalAntal, this.kassationPct.toFixed(1) + '%', '100.0%'] as any);
    const csv = [headers, ...rows].map(r => r.join(';')).join('\n');
    const blob = new Blob(['\uFEFF' + csv], { type: 'text/csv;charset=utf-8;' });
    const url = URL.createObjectURL(blob);
    const a = document.createElement('a');
    a.href = url; a.download = `kassationsanalys-${new Date().toISOString().slice(0, 10)}.csv`;
    ...
    this.kassationParetoChart = new Chart(ctx, {
      ...
      options: {
        ...
        scales: {
          ...
          y: { beginAtZero: true, ticks: { color: '#a0aec0' }, grid: { color: 'rgba(255,255,255,0.06)' },
            title: { display: true, text: 'Antal', color: '#a0aec0' } },
          yRight: { position: 'right', min: 0, max: 100,
      ...
      plugins: [{
        ...
        afterDraw(chart: any) {
          ...
          c2.moveTo(xAx.left, y80); c2.lineTo(xAx.right, y80);
          c2.strokeStyle = '#e53e3e'; c2.lineWidth = 1.5; c2.setLineDash([6, 4]);
          c2.stroke(); c2.setLineDash([]);
          c2.fillStyle = '#e53e3e'; c2.font = '11px sans-serif';
