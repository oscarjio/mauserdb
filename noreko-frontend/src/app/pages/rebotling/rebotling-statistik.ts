...
import { RebotlingService, OEETrendDay, WeekComparisonDay, BestShift, CycleHistogramResponse, SPCResponse, ChartAnnotation, QualityTrendDay, QualityTrendResponse, OeeWaterfallResponse, WeekdayStatsEntry, ProductionEvent, CycleByOperatorEntry, CycleByOperatorResponse, RejectionAnalysisResponse, RejectionTrendDay, RejectionParetoItem } from '../../services/rebotling.service';
...
export class RebotlingStatistikPage implements OnInit, AfterViewInit, OnDestroy {
  ...
  // ======== IBC KVALITETSANALYS DEEP-DIVE ========
  rejectionAnalysisDays: number = 30;
  rejectionAnalysisLoading: boolean = false;
  rejectionAnalysisLoaded: boolean = false;
  rejectionKpi: {
  ...
  rejectionHasParetoData: boolean = false;
  rejectionTotalKassation: number = 0;
  ...
  ngOnInit() {
    ...
    this.loadKassationPareto();
    this.loadOeeComponents();
  ...
  private buildParetoChart(): void {
    ...
    // Färglägg staplar: mörkblå = vital (kumulativ <= 80%), grå = trivial
    ...
    this.paretoChart = new Chart(ctx, {
      ...
      options: {
        ...
        plugins: {
          legend: {
            display: true,
            labels: { color: '#a0aec0', boxWidth: 14 }
          },
          tooltip: {
            callbacks: {
              label: (ctx: any) => {
          ...
          // Streckad 80%-referenslinje via afterDraw plugin (inline)
        },
        scales: {
          x: {
            ticks: {
      ...
      plugins: [{
        ...
        afterDraw(chart: any) {
          ...
          ctx2.save();
          ctx2.beginPath();
          ctx2.moveTo(xAxis.left, y80);
          ctx2.lineTo(xAxis.right, y80);
  ...
  loadKassationPareto(): void {
    ...
    this.http.get<any>(
      `/noreko-backend/api.php?action=rebotling&run=kassation-pareto&days=${this.kassationDays}`,
    ...
    .subscribe((res: any) => {
      ...
      if (res?.success) {
        this.kassationPareto        = res.pareto || [];
        this.kassationTotalAntal    = res.total_kassation ?? 0;
        this.kassationTotalProduktion = res.total_produktion ?? 0;
        this.kassationPct           = res.kassation_pct ?? 0;
        setTimeout(() => this.buildKassationParetoChart(), 0);
      }
    });
  }

  onKassationDaysChange(): void {
    this.loadKassationPareto();
  }
  ...
  private buildKassationParetoChart(): void {
    ...
    const labels      = this.kassationPareto.map(p => p.namn);
    const values      = this.kassationPareto.map(p => p.antal);
    const kumulativ   = this.kassationPareto.map(p => p.kumulativ_pct);

    const maxVal = Math.max(...values, 1);

    this.kassationParetoChart = new Chart(ctx, {
      type: 'bar',
      data: {
        labels,
        datasets: [
        ...
        plugins: {
          ...
          tooltip: {
            callbacks: {
              label: (ctx: any) => {
                if (ctx.datasetIndex === 0) {
                  const item = this.kassationPareto[ctx.dataIndex];
                  return [`Antal: ${item.antal}`, `Andel: ${item.pct}%`];
                }
      ...
      plugins: [{
        ...
        afterDraw(chart: any) {
          ...
          c2.save();
          c2.beginPath();
          c2.moveTo(xAx.left, y80);
          c2.lineTo(xAx.right, y80);
          c2.strokeStyle = '#e53e3e';
          c2.lineWidth = 1.5;
          c2.setLineDash([6, 4]);
          c2.stroke();
          c2.setLineDash([]);
          ...
          c2.fillText('80%', xAx.right - 32, y80 - 5);
          c2.restore();
        }
      }]
    });
  ...
  // ======== OEE-KOMPONENTER: LADDA OCH BYGG GRAF ========

  loadOeeComponents(): void {
    this.oeeComponentsLoading = true;
    this.http.get<any>(
      `/noreko-backend/api.php?action=rebotling&run=oee-components&days=${this.oeeComponentsDays}`,
      { withCredentials: true }
    )
      .pipe(timeout(10000), catchError(() => of(null)), takeUntil(this.destroy$))
      .subscribe(res => {
        this.oeeComponentsLoading = false;
        if (res?.success) {
          this.oeeComponentsData = res.data || [];
          this.buildOeeComponentsChart();
        }
      });
  }
  ...
  buildOeeComponentsChart(): void {
    ...
    const labels = this.oeeComponentsData.map(d => {
      const p = d.datum.split('-');
      return `${p[2]}/${p[1]}`;
    });

    this.oeeComponentsChart = new Chart(ctx, {
      type: 'line',
      data: {
        labels,
        datasets: [
          {
            label: 'Tillgänglighet %',
            data: this.oeeComponentsData.map(d => d.tillganglighet),
          ...
          {
            label: 'Kvalitet %',
            data: this.oeeComponentsData.map(d => d.kvalitet),
            borderColor: 'rgba(99, 179, 237, 1)',
          ...
          {
            label: 'WCM 85%',
            data: this.oeeComponentsData.map(() => 85),
  ...
  get rejectionTrendColor(): string {
    const t = this.rejectionKpi.trend_vs_forra_veckan;
    if (t === 'up') return '#48bb78';
    if (t === 'down') return '#fc8181';
    return '#a0aec0';
  }
            ...
            callbacks: {
              label: (ctx: any) => {
                ...
                if (ctx.datasetIndex === 0) {
                  ...
                  return [
                    `Kvalitet: ${v}%`,
                    `OK: ${d?.ibc_ok ?? 0}  Kasserade: ${d?.ibc_kasserade ?? 0}`
                  ];
                }
  ...
  exportRejectionCSV(): void {
    ...
    const a = document.createElement('a');
    a.href = url;
    a.download = `kvalitetsanalys-${new Date().toISOString().slice(0, 10)}.csv`;
    a.click();
    URL.revokeObjectURL(url);
  }
