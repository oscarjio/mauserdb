...
## BUG-009: NewsController — MAX(ibc_ok) per dag underskattar flerskiftsdagar (EJ FIXAD)
**Rapporterad:** 2026-05-16  
**Status:** EJ åtgärdad  
**Symptom:** Nyhetsflödets "Rekordag", "Produktionsrekord" och "Senaste produktionsdagar"-events visar fel IBC-tal på dagar med fler än ett skift — visar bara sista skiftets MAX istället för summerade deltas.  
**Rotorsak:** `NewsController.php` rad 289–306, 342–348, 448–452, 481–494, 527–533 använder `MAX(ibc_ok) GROUP BY DATE(datum)` utan LAG()-delta.  
**Filer:** `noreko-backend/classes/NewsController.php`  
**Fix:** LAG()-CTE per (datum, skiftraknare) → delta → SUM per dag.

---

## BUG-009: NewsController — rekordag/produktion-events underskattar IBC på flerskiftsdagar (EJ FIXAD)
**Rapporterad:** 2026-05-16  
**Status:** EJ åtgärdad  
**Symptom:** "Rekordag", "Produktionsrekord" och "Senaste produktionsdagar"-events på nyhetsflödet visar för lågt IBC-tal på dagar med fler än ett skift. Exempel: En dag med 2 skift à 20 IBC vardera kan visas som "20 IBC" (bara sista skiftets MAX) istället för "40 IBC".  
**Rotorsak:** `NewsController::getEvents()` använder `MAX(ibc_ok) GROUP BY DATE(datum)` direkt mot `rebotling_ibc`. Eftersom `ibc_ok` nollställs per skift (inte per dag) ger `MAX` bara det sista skiftets slutvärde — resten av dagsprodukterna räknas inte med. OEE-beräkningarna (`MAX(ibc_ok) / (MAX(ibc_ok) + MAX(ibc_ej_ok)) * 100`) är likaså felaktiga på flerskiftsdagar.  
**Filer:** `noreko-backend/classes/NewsController.php` rad 289–306, 342–348, 448–452, 481–494, 527–533  
**Fix (ej implementerad):** Ersätt `MAX(ibc_ok) GROUP BY DATE(datum)` med ett CTE som gör `MAX(ibc_ok) GROUP BY DATE(datum), skiftraknare` + LAG()-delta per skiftraknare, och summerar deltavärdena per dag. Alternativt: JOIN mot `rebotling_skiftrapport` för att hämta dagstotaler direkt.
